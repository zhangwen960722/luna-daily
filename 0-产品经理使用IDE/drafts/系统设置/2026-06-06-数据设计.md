...
> **版本**: v2.0 | **日期**: 2026-06-06 | **作者**: AI PM
> **关联 RDD**: `2026-06-06-用户需求.md` v2.0
...
| 实体名称 | 对应表/子表 | 映射方式 | 说明 |
|----------|------------|---------|------|
| 客户账号（CustomerAccount） | `customer_account` | 独立表 | 系统设置核心聚合根，管理客户入驻全生命周期 |
| 合同（Contract） | `customer_contract` | 1:1 子表 | 通过 `account_id` 关联 customer_account |
| 开户申请（AccountApplication） | `account_application` | 1:N 子表 | 通过 `account_id` 关联 customer_account；拒绝后重新提交产生新记录 |
| 开户附件（ApplicationAttachment） | `application_attachment` | 1:N 子表 | 通过 `application_id` 关联 account_application |
| 邮件通知记录（EmailNotification） | `email_notification` | 1:N 子表 | 通过 `account_id` 关联 customer_account；记录每次邮件发送 |
| 员工（Employee） | `employee` | 独立表 | 内部员工管理 |
| 角色（Role） | `role` | 独立表 | RBAC 角色定义 |
| 资源（Resource） | `resource` | 独立表 | 菜单+按钮树形结构 |
...
**表名**: `customer_account` | **对应实体**: CustomerAccount

> **设计说明**: 存储客户登录账号信息及生命周期状态。客户公司主体信息从客户主数据（外部模块）引用，本表仅维护账号级属性。customer_region 决定入驻路径分流。
...
| `account_code` | 虚拟账户 | String(64) | Yes | **Unique** | 系统自动生成，格式: K{customerId}_admin |
| `customer_region` | 客户属地 | TinyInt | Yes | — | 10:大陆, 20:香港。决定入驻路径分流 |
| `email` | 账号接收邮箱 | String(128) | Yes | — | 选择客户后自动带出（取客户联系人中系统账号接收人） |
| `user_type` | 用户类型 | TinyInt | Yes | — | 10: customer（客户） |
| `business_types` | 开通业务 | JSON | Yes | — | `["TMS","WMS"]` 格式，多选 |
| `account_status` | 账户状态 | TinyInt | Yes | Index | 10:待签约, 20:待开户, 30:待审核, 40:已拒绝, 50:正常, 60:已冻结 |
| `contract_status` | 合同状态 | TinyInt | Yes | — | 10:未签约, 20:签约中, 30:已签约, 40:已过期（系统自动判断，不持久化） |
| `contract_start_date` | 合同开始日期 | Date | No | — | 签约后填充 |
| `contract_end_date` | 合同结束日期 | Date | No | — | 签约后填充 |
...
**关联关系**:
- `One-to-One` with `customer_contract` (通过 `id` → `account_id`)
...
**表名**: `customer_contract` | **对应实体**: Contract

> **设计说明**: 每个客户账号对应一份有效合同（1:1）。签约完成后合同数据写入本表，customer_account 状态同步更新。

| 字段名 (En) | 字段名 (Cn) | 类型 (Type) | 必填 | 约束/索引 | 枚举/备注 |
| :---------- | :---------- | :------------ | :--- | :--------- | :------------------------------------- |
| `id` | 主键 | BigInt | Yes | **PK** | 雪花ID |
| `tenant_id` | 租户ID | String(32) | Yes | Index | SaaS 数据隔离 |
| `account_id` | 客户账号ID | BigInt | Yes | **FK, Unique** | 关联 customer_account.id |
| `is_simple` | 是否简易合同 | TinyInt | Yes | — | 10:是, 20:否 |
| `is_standard` | 是否标准合同 | TinyInt | Yes | — | 10:是, 20:否 |
| `company_title` | 我司合同抬头 | String(128) | Yes | — | 如"广东省飞点跨境供应链有限公司" |
| `payment_term` | 账期 | TinyInt | No | — | 10:月结, 20:周结。标准合同时必填 |
| `contract_years` | 合同有效期年限 | TinyInt | No | — | 1/2/3 |
| `start_date` | 合同开始日期 | Date | No | — | 非简易+非标准时/HK特殊时必填 |
| `end_date` | 合同结束日期 | Date | No | — | 同上 |
| `sign_date` | 签订日期 | Date | No | — | 简易合同时必填 |
| `modify_content` | 合同修改内容 | Text | No | — | 非标准合同时必填 |
| `contract_type` | 合同类型 | TinyInt | No | — | 10:普通, 20:香港特殊。根据客户所在地区自动识别 |
| `attachment_url` | 合同附件URL | String(512) | No | — | 存储对象存储路径 |
| `created_at` | 创建时间 | DateTime | Yes | — | 自动生成 |
| `created_by` | 创建人 | String(32) | Yes | — | 操作员工ID |
| `updated_at` | 更新时间 | DateTime | Yes | — | 自动维护 |
| `updated_by` | 更新人 | String(32) | Yes | — | 操作员工ID |
...
**关联关系**:
- `One-to-One` with `customer_account` (通过 `account_id`)

---

#### 2.3 开户申请 (account_application)

**表名**: `account_application` | **对应实体**: AccountApplication

> **设计说明**: 存储客户开户时填写的18个字段表单。每个 account 可能有多条历史申请记录（拒绝后重新提交时创建新记录）。审核通过后此表数据可作为客户画像数据被其他模块引用。company_name、customer_type、customer_region 从 customer_account 带出，存储为只读快照。
...
**表名**: `application_attachment` | **对应实体**: ApplicationAttachment
...
**表名**: `email_notification` | **对应实体**: EmailNotification
...
| `sender` | 发件箱 | String(128) | Yes | — | 系统配置的发送邮箱 |
| `recipient` | 收件人 | String(128) | Yes | — | 客户账号接收邮箱 |
| `subject` | 邮件主题 | String(256) | Yes | — | 模板主题 |
| `body` | 邮件内容 | Text | Yes | — | 含虚拟账号和初始密码的邮件正文 |
...
| `type` | 资源类型 | TinyInt | Yes | — | 10:menu（菜单）, 20:button（按钮） |
...
**表名**: `role_employee` | **对应实体**: RoleEmployee (N:N 关联)
...
**表名**: `role_resource` | **对应实体**: RoleResource (N:N 关联)
...
```mermaid
erDiagram
    customer_account ||--|| customer_contract : "1:1 (account_id)"
...
- 并发场景：多个管理员同时编辑同一客户账号/员工/角色时，后提交者收到版本冲突提示
...
- 客户主数据（Customer Master）：`customer_account.customer_ref_id` 引用外部客户管理模块，本系统不存储客户主体信息（如统一社会信用代码、法人等），仅存储账号级属性
...
- `customer_account.contract_status` 中的"已过期"（40）状态不在签约时写入，而是每次读取列表时由后端或前端根据 `contract_end_date` 与当前日期对比动态判断
- 仅当 `contract_status` 为"已签约"（30）且当前日期 > `contract_end_date` 时，展示为"已过期"
...
- 提交档案时，香港企业（20）→ `account_status = 20`（待开户），`contract_status = 10`（未签约）
