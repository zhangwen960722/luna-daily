...
| contractCount | 合同数量 | 按钮/Popover | 始终显示；显示该客户当前合同数量（如"2份合同"）；点击弹出Popover展示合同明细（签约主体+合同签署状态+合同开始日期+合同结束日期） |
...
| 否 | 否 | 合同开始日期(Date:必填) + 合同结束日期(Date:必填) + 合同修改内容(Textarea) + 附件上传(支持多个) |
| 是 | 是 | 账期(Select:联动域名+属地) + 合同有效期限(Select:1/2/3年) + 签订日期(Date:默认当天) |
| 是 | 否 | 合同有效期限(Select:1/2/3年) + 签订日期(Date:默认当天) + 合同开始日期(Date:必填) + 合同结束日期(Date:必填) + 合同修改内容(Textarea) + 附件上传 |
