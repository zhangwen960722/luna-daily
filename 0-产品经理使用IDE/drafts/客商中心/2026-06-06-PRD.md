...
- 版本号：v1.3
- 状态：草稿
- 作者：AI PM（v1.3: 客商中心×货主端数据融合 — 融合接口+联动规则+跨模块验收标准）
- 评审人：产品/研发/测试/业务代表
...
- 用户需求(RDD)：`drafts/客商中心/2026-06-06-用户需求.md`
- 数据设计：`drafts/客商中心/2026-06-06-数据设计.md`
- 需求背景：无
...
|--------|----|--------|------|-------------|
| CustomerType | 10 | PERSONAL | 自然人 | customer.customer_type |
| CustomerType | 20 | ENTERPRISE | 企业 | customer.customer_type |
| EnterpriseAttribute | 10 | MAINLAND | 中国大陆 | customer.enterprise_attribute, enterprise_profile.enterprise_attribute, personal_profile.personal_region_type |
| EnterpriseAttribute | 20 | HONGKONG | 中国香港 | 同上 |
| YesNo | 10 | YES | 是 | customer.is_peer, supplier.is_peer |
| YesNo | 20 | NO | 否 | 同上 |
| SignStatus | 10 | UNSIGNED | 未签署 | customer.sign_status |
| SignStatus | 20 | SIGNING | 签署中 | customer.sign_status |
| SignStatus | 30 | SIGNED | 已签署 | customer.sign_status |
| SignStatus | 40 | EXPIRED | 已过期 | customer.sign_status |
| ServiceStatus | 10 | NORMAL | 正常 | customer.service_status, supplier.service_status, supplier_print_service.status, level.status |
| ServiceStatus | 20 | FROZEN | 已冻结 | 同上 |
| BillingCycle | 10 | FIXED | 固定 | customer.billing_cycle |
| BillingCycle | 20 | UNFIXED | 不固定 | customer.billing_cycle |
| PaymentTerm | 101 | MONTHLY | 月结 | customer.payment_term（固定） |
| PaymentTerm | 102 | BIMONTHLY | 双月结 | customer.payment_term（固定） |
| PaymentTerm | 103 | TRIMONTHLY | 三月结 | customer.payment_term（固定） |
| PaymentTerm | 201 | WEEKLY | 周结 | customer.payment_term（不固定） |
| PaymentTerm | 202 | PRE_PORT | 到港前结 | customer.payment_term（不固定） |
| PaymentTerm | 203 | SIGN_MONTHLY | 签收月结 | customer.payment_term（不固定） |
| PaymentTerm | 204 | OVERSEAS_WAREHOUSE | 到海外仓结 | customer.payment_term（不固定） |
| PaymentTerm | 205 | SIGN_SETTLE | 签收结 | customer.payment_term（不固定） |
| SettlementType | 10 | BY_MONTH | 按月 | customer.settlement_type |
| SettlementType | 20 | BY_DAY | 按天 | customer.settlement_type |
| LevelCode | — | SS/S/A/B/C/D/E/F/G | SS~G | customer.customer_level, supplier.level, level.level_code |
| LevelType | 10 | CUSTOMER | 客户等级 | level.level_type |
| LevelType | 20 | SUPPLIER | 供应商等级 | level.level_type |
| SupplierCategory | 100 | MAINLINE | 干线运输供应商 | supplier.supplier_category[] |
| SupplierCategory | 200 | CARGO_COLLECTION | 揽货服务商 | supplier.supplier_category[] |
| SupplierCategory | 300 | WAREHOUSE | 仓储供应商 | supplier.supplier_category[] |
| SupplierCategory | 400 | CUSTOMS | 关务供应商 | supplier.supplier_category[] |
| SupplierCategory | 500 | LAST_MILE | 尾程运输供应商 | supplier.supplier_category[] |
| SupplierCategory | 600 | AGENCY | 综合代理 | supplier.supplier_category[] |
| SupplierCategory | 900 | OTHER | 其他 | supplier.supplier_category[] |
| SupplierDetail | 101 | TRUCKING | 汽运 | supplier.supplier_detail[] |
| SupplierDetail | 102 | SEA | 海运 | supplier.supplier_detail[] |
| SupplierDetail | 103 | AIR | 空运 | supplier.supplier_detail[] |
| SupplierDetail | 104 | RAILWAY | 铁运 | supplier.supplier_detail[] |
| SupplierDetail | 201 | HUOLALA | 货拉拉 | supplier.supplier_detail[] |
| SupplierDetail | 202 | TRAILER | 拖车行 | supplier.supplier_detail[] |
| SupplierDetail | 301 | OVERSEAS_TRANSIT | 海外中转仓 | supplier.supplier_detail[] |
| SupplierDetail | 302 | BONDED | 保税仓 | supplier.supplier_detail[] |
| SupplierDetail | 303 | LOCAL | 本地仓 | supplier.supplier_detail[] |
| SupplierDetail | 304 | RETURN | 退件仓 | supplier.supplier_detail[] |
| SupplierDetail | 401 | DECLARATION | 报关行 | supplier.supplier_detail[] |
| SupplierDetail | 402 | CLEARANCE | 清关行 | supplier.supplier_detail[] |
| SupplierDetail | 403 | INSPECTION | 查验代理 | supplier.supplier_detail[] |
| SupplierDetail | 501 | EXPRESS | 快递 | supplier.supplier_detail[] |
| SupplierDetail | 502 | TRUCK | 卡车 | supplier.supplier_detail[] |
| SupplierDetail | 601 | PEER | 同行 | supplier.supplier_detail[] |
| SupplierDetail | 901 | OTHER_DETAIL | 其他 | supplier.supplier_detail[] |
| ContactRole | 10 | COMPANY_HEAD | 公司负责人 | customer_contact.role, profile_contact.role |
| ContactRole | 20 | LOGISTICS_CONTACT | 物流联系人 | 同上 |
| ContactRole | 30 | FINANCE_HEAD | 财务负责人 | 同上 |
| MaterialType | 10 | BUSINESS_LICENSE | 营业执照 | customer_material.material_type |
| MaterialType | 20 | ID_CARD | 法人身份证正反面 | customer_material.material_type |
| MaterialType | 30 | TIANYANCHA | 天眼查风险信息PDF | customer_material.material_type |
| MaterialType | 99 | CUSTOM | 自定义 | customer_material.material_type |
| AddressType | 10 | PICKUP | 取件地址 | customer_address.address_type |
| AddressType | 20 | RECEIVE | 收货地址 | customer_address.address_type |
| CommType | 10 | WECOM | 企业微信 | customer_communication.comm_type |
| Channel | 10 | TANJI | 探迹 | enterprise_profile.channel, personal_profile.channel |
| Channel | 20 | EXHIBITION | 展会 | 同上 |
| Channel | 30 | BOSS_INTRO | 老板介绍 | 同上 |
| Channel | 40 | FRIEND | 朋友推荐 | 同上 |
| CardSide | 10 | FRONT | 人像面 | customer_material_file.card_side |
| CardSide | 20 | BACK | 国徽面 | customer_material_file.card_side |
| ProfileStatus | 10 | DRAFT | 草稿 | enterprise_profile.status, personal_profile.status |
| ProfileStatus | 20 | PENDING | 待审核 | 同上 |
| ProfileStatus | 30 | APPROVED | 已通过 | 同上 |
| ProfileStatus | 40 | REJECTED | 已驳回 | 同上 |
| ProfileType | 10 | ENTERPRISE | 企业档案 | pickup_address.profile_type, profile_contact.profile_type |
| ProfileType | 20 | PERSONAL | 自然人档案 | 同上 |
| InvitationStatus | 10 | VALID | 有效 | invitation_link.status |
| InvitationStatus | 20 | USED | 已使用 | invitation_link.status |
| InvitationStatus | 30 | EXPIRED | 已过期 | invitation_link.status |
| SigningMode | 10 | MAINLAND_ONLINE | 大陆线上签约 | customer_contract.signing_mode |
| SigningMode | 20 | HK_OFFLINE | 香港线下签约 | customer_contract.signing_mode |
| ApprovalType | 10 | SIMPLE | 简易 | feishu_approval.approval_type, customer_contract.feishu_approval_type |
| ApprovalType | 20 | MOXIAN | 墨线 | 同上 |
| ApprovalType | 30 | MEIXIAN | 美线 | 同上 |
| ApprovalStatus | 10 | IN_PROGRESS | 审批中 | feishu_approval.approval_status |
| ApprovalStatus | 20 | APPROVED | 已通过 | feishu_approval.approval_status |
| ApprovalStatus | 30 | REJECTED | 已驳回 | feishu_approval.approval_status |
| ApprovalStatus | 40 | CANCELLED | 已撤销 | feishu_approval.approval_status |
| FileSource | 10 | MANUAL | 手工上传 | customer_material_file.source |
| FileSource | 20 | ESIGN | E签宝回传 | customer_material_file.source |
| FileSource | 30 | OCR | OCR识别 | customer_material_file.source |
| FileSource | 40 | TIANYANCHA | 天眼查生成 | customer_material_file.source |
...
| 当前状态 | 操作 | 目标状态 | 触发角色 | 校验条件 |
|---------|------|---------|---------|---------|
| 未签署 | 发起签约 | 签署中 | 销售/客服 | serviceStatus=正常 |
| 已过期 | 发起签约 | 签署中 | 销售/客服 | serviceStatus=正常 |
| 签署中 | 大陆：飞书审批通过+E签宝签署完成 | 已签署 | 系统自动 | — |
| 签署中 | 香港：上传合同附件+保存 | 已签署 | 销售/客服 | 附件至少1个 |
| 已签署 | 合同到期 | 已过期 | 系统定时任务 | contract_end_date < now() |
...
| 供应商管理 | 新增/编辑供应商 — 4个Tab（基本信息/联系人/资料/打单服务） | P0 | 供应商管理.html（弹窗） | 编辑页 |
| 供应商管理 | 大类明细级联 + SCAC条件必填（海运/快递/卡车） | P0 | 供应商管理.html（弹窗） | 表单联动 |
...
|--------|--------|------|------|--------|---------|------|
| customerId | 关联客户 | Select | — | — | — | 可选，关联已有客户记录 |
| expireDays | 有效天数 | Select | — | 7 | — | 7/14/30天 |
...
| 字段名 | 中文名 | 类型 | 必填 | 说明 |
|--------|--------|------|------|------|
| enterpriseAttribute | 企业属地 | Select | ✅ | 中国大陆/中国香港（默认大陆） |
| licenseUpload | 营业执照 | Upload | — | 上传后触发OCR自动识别公司名称、注册号、信用代码 |
| companyName | 公司名称 | Input | ✅ | OCR自动填充，可修改 |
| registrationNo | 公司注册号码 | Input | ✅ | OCR自动填充，可修改 |
| creditCode | 统一社会信用代码 | Input | ✅ | OCR自动填充，可修改；提交时全局唯一校验 |
| companyProvince | 公司注册地址-省 | Select | ✅ | — |
| companyCity | 公司注册地址-市 | Select | ✅ | 与省级联 |
| companyDetailAddress | 公司注册地址-详细 | Input | ✅ | — |

**卡片2 — 取货地址**：

| 字段名 | 中文名 | 类型 | 必填 |
|--------|--------|------|------|
| pickupProvince | 取货地址-省份 | Select | ✅ |
| pickupCity | 取货地址-城市 | Select | ✅ |
| pickupDetail | 取货地址-详细 | Input | ✅ |
...
| 字段名 | 中文名 | 类型 | 必填 | 联动 |
|--------|--------|------|------|------|
| channel | 添加渠道 | Select | ✅ | 探迹/展会/老板介绍/朋友推荐 |
| mainCountry | 主营业务国 | Select | ✅ | 美国/英国/德国等 |
| mainPlatform | 主要经营平台 | Select(multiple) | ✅ | 与业务国级联选择 |

**卡片5 — 法人证件上传**：

| 字段名 | 中文名 | 类型 | 必填 | 校验 |
|--------|--------|------|------|------|
| idCardFrontUrl | 法人身份证人像面 | Upload(大图) | ✅ | ≤2MB, jpg/png/jpeg |
| idCardBackUrl | 法人身份证国徽面 | Upload(大图) | ✅ | ≤2MB, jpg/png/jpeg |
| remarks | 客户备注 | Textarea | — | — |
...
| 字段名 | 中文名 | 类型 |
|--------|--------|------|
| customerName | 客户名称 | Input（模糊搜索） |
| signStatus | 合同签署状态 | Select(multiple)：已签署/签署中/已过期 |
| customerType | 客户类型 | Select(multiple)：企业/自然人 |
| enterpriseAttribute | 客户属地 | Select(multiple)：中国大陆/中国香港 |
| isPeer | 是否同行 | Select(multiple)：是/否 |
...
| 6 | 客服代表 | 文本，多选用"、"连接 |
| 7 | 客户等级 | 文本：SS~G |
| 8 | 合同签署状态 | Tag：已签署(success)/签署中(warning)/已过期(danger)/未签署(info) |
| 9 | 合同开始日期 | 文本 |
| 10 | 合同结束日期 | 文本 |
| 11 | 服务状态 | Tag：正常(success)/已冻结(danger) |
| 12 | 账期 | 文本：月结/双月结/周结等 |
| 13 | 首次下单时间 | 文本 |
| 14 | 未下单天数 | 文本 |
| 15 | 操作 | 按钮：编辑 / 签约 / 冻结或启用 |
...
| 签约 | signStatus∈{未签署, 已过期} AND serviceStatus=正常 | 大陆→线上签约弹窗；香港→HK线下签约弹窗 |
| 冻结 | serviceStatus=正常 | 二次确认弹窗（type:warning）→ 冻结 + 关联账户同步冻结 |
...
|--------|--------|------|------|--------|------|
| customerCode | 客户编号 | Input(disabled) | — | K+1+0001自增 | 编辑时不可改 |
| customerType | 客户类型 | Select | ✅ | — | 编辑时不可改 |
| customerName | 客户名称 | Input | ✅ | — | 编辑时不可改 |
| creditCode | 统一社会信用代码 | Input | 条件 | — | 企业必填+唯一校验；自然人禁用；编辑时不可改 |
| enterpriseAttribute | 客户属地 | Select | ✅ | — | 编辑时不可改 |
| isPeer | 是否同行 | Select | ✅ | — | — |
| customerLevel | 客户等级 | Select | ✅ | G | SS~G，来源等级管理 |
| csRepresentative | 客服代表 | Select(multiple) | ✅ | — | — |
| salesRepresentative | 销售代表 | Select | ✅ | — | — |
| salesSource | 销售来源 | Input | — | — | — |
| businessTypes | 开通业务 | Select(multiple) | ✅ | — | TMS/WMS |

**更多信息 Tab（7个有效+2个占位）**：

1. **账期 Tab**：billingCycle → paymentTerm（联动）→ settlementType（自动推导）→ daysOrDate（≥0整数）
2. **资料 Tab**：营业执照（企业必填）+ 法人身份证正反面（始终必传）+ 天眼查风险信息（选传）+ 自定义附件
3. **服务合同 Tab**：contractStartDate + contractEndDate + contractFiles（PDF/图片）
4. **地址库 Tab**：只读表格，来源于客户档案，空数据显示空状态
5. **联系人 Tab**：固定3行（公司负责人/物流联系人/财务负责人），每行含姓名/电话/邮箱，指定合同对接人+系统账号接收人
6. **通信信息 Tab**：commType（固定"企业微信"）+ groupId（必填）
7. **关联账户 Tab**：只读表格，空数据显示空状态
8. **评分详情 Tab**（占位）：空状态
9. **开票抬头 Tab**（占位）：空状态
...
| 保存并生成账户 | 需权限 → 额外校验身份证完整性+营业执照(企业) → 保存 + 开通账户 + 邮件通知 → Toast "保存成功，并已为该客户生成系统账户！" → 合同状态=未签署 |
...
|----------|-----------|---------|
| 否 | 是 | 是否简易/是否标准 + 我司合同抬头(自动) + 签约账期(域名+属地联动) + 合同期限(1/2/3年) + 开始日期(默认当天) + 结束日期(自动计算) |
| 否 | 否 | 是否简易/是否标准 + 我司合同抬头(自动) + 合同修改内容(textarea) + 附件 |
| 是 | 是 | 是否简易/是否标准 + 我司合同抬头(自动) + 签约账期 + 合同期限(1/2/3年) + 签订日期(默认当天) |
| 是 | 否 | 是否简易/是否标准 + 我司合同抬头(自动) + 合同期限(1/2/3年) + 签订日期(默认当天) + 合同修改内容 + 附件 |

**提交后流程**：
1. 创建 customer_contract 记录（sign_status=签署中）
...
**合同信息回写规则**：
- 标准合同（is_contract_auto_sync=true）：customer.sign_status=已签署 + contract_start_date/end_date 取签约值自动写入
- 非标合同（is_contract_auto_sync=false）：销售手工在客户编辑页 > 服务合同 Tab 中维护
...
| 字段名 | 中文名 | 类型 | 必填 | 校验 |
|--------|--------|------|------|------|
| contractStartDate | 合同开始日期 | DatePicker | ✅ | — |
| contractEndDate | 合同结束日期 | DatePicker | ✅ | 不可早于开始日期 |
| contractFiles | 服务合同协议 | Upload | ✅ | PDF/图片，至少1个附件 |

**提交后**：保存 → customer.sign_status=已签署 → Toast "合同签约成功！"
...
| 字段名 | 中文名 | 必填 | 备注 |
|--------|--------|------|------|
| supplierName | 供应商名称 | — | disabled，继承自客户名称 |
| supplierCategory | 供应商大类 | ✅ | 多选 |
| supplierDetail | 供应商明细 | ✅ | 多选，跟随大类联动 |
| scac | SCAC | 条件 | 明细含快递时必填 |
| level | 供应商等级 | ✅ | SS~G |
...
| 字段名 | 中文名 | 类型 |
|--------|--------|------|
| supplierName | 供应商名称 | Input（模糊搜索） |
| supplierCategory | 供应商大类 | Select(multiple) |
| supplierDetail | 供应商明细 | Select(multiple)（跟随大类联动） |
| isPeer | 是否同行 | Select(multiple)：是/否 |
...
| 字段名 | 中文名 | 类型 | 必填 | 默认值 | 校验规则 | 备注 |
|--------|--------|------|------|--------|---------|------|
| supplierCode | 供应商编号 | Input(disabled) | — | G+10001自增 | — | 编辑时不可改 |
| isPeer | 是否同行 | Select | ✅ | 否 | — | 是/否 |
| supplierName | 供应商名称 | Input | ✅ | — | 全局唯一 | 编辑时不可改 |
| supplierShortName | 供应商简称 | Input | ✅ | — | — | — |
| supplierCategory | 供应商大类 | Select(multiple) | ✅ | — | — | 7大类：干线运输供应商/揽货服务商/仓储供应商/关务供应商/尾程运输供应商/综合代理/其他 |
| supplierDetail | 供应商明细 | Select(multiple) | ✅ | — | 与大类级联 | 跟随大类联动，大类切换→明细清空 |
| scac | SCAC代码 | Input | 条件 | — | — | 明细含"海运/快递/卡车"时必填 |
| level | 供应商等级 | Select | ✅ | G | — | 下拉取等级管理-供应商等级Tab中状态=正常的等级 |
...
| 字段名 | 中文名 | 类型 | 必填 | 默认值 | 校验规则 | 备注 |
|--------|--------|------|------|--------|---------|------|
| customerType | 客户类型 | Select | ✅ | — | — | 自然人/企业 |
| customerName | 客户名称 | Input(disabled) | — | 取供应商名称 | — | 不可修改 |
| creditCode | 统一社会信用代码 | Input | 条件 | — | 全局唯一 | 企业必填，自然人禁用 |
| enterpriseAttribute | 客户属地 | Select | ✅ | — | — | 中国大陆/中国香港 |
| isPeer | 是否同行 | Select | ✅ | 取供应商值 | — | 是/否 |
| customerLevel | 客户等级 | Select | ✅ | G | — | SS~G |
| csRepresentative | 客服代表 | Select(multiple) | ✅ | — | — | — |
| salesRepresentative | 销售代表 | Select | ✅ | — | — | — |
| salesSource | 销售来源 | Input | — | — | — | — |
| businessTypes | 开通业务 | Select(multiple) | ✅ | — | — | TMS/WMS |
...
**Tab4 — 通信信息**：commType（固定"企业微信"）+ groupId（必填）。
...
|--------|--------|------|------|---------|------|
| levelCode | 级别代码 | Select | ✅ | 同Tab内level_code唯一 | 底纹提示A/B/C/D/E/F/G/S/SS |
| status | 状态 | Radio | ✅ | — | 启用(正常) / 冻结 |
...
| R21 | 保存并生成账户 | → 生成账户 + 邮件通知 | 合同状态=未签署 | Toast "保存成功，并已为该客户生成系统账户！" |
| R22 | 账单周期=固定 | 可选月结/双月结/三月结 | settlementType=按月 | 切换周期→联动重置 |
| R23 | 账单周期=不固定 | 可选周结/到港前结/签收月结/到海外仓结/签收结 | settlementType=按天 | 切换周期→联动重置 |
| R24 | daysOrDate | 必须≥0整数 | — | Toast "必须为大于等于0的整数" |
| R25 | 大陆签约 | isSimple x isStandard 4分支 | 显示不同字段组合 | — |
| R26 | 我司合同抬头 | 根据域名自动填充 | — | — |
| R27 | 签约账期可选值 | 根据域名和企业属地动态计算 | — | — |
| R28 | 合同开始日期 | 默认当天 | — | — |
| R29 | 合同结束日期 | 开始日期 + 合同期限(1/2/3年) | 自动计算 | — |
| R30 | 签订日期 | 默认当天 | — | — |
| R31 | 发起签约 | 调用飞书审批 | feishu_approval 创建 | 审批类型根据合同自动路由(简易/墨线/美线) |
| R32 | 飞书审批通过 | 调用E签宝生成合同 | esign_contract_id 回填 | — |
| R33 | E签宝签署完成 | esign_file_url 回填 | 合同附件回传 | — |
| R34 | 合同信息回写 | 标准合同→取签约值自动写入；非标合同→手工维护 | customer 表更新 | — |
| R35 | 香港签约 | 上传合同附件+保存 | sign_status=已签署 | 附件至少1个 |
| R36 | 供应商大类切换 | supplierDetail 清空+联动 | — | — |
| R37 | 明细含海运/快递/卡车 | scac 必填 | — | Toast "选择海运/快递/卡车时，此项必填" |
...
| R45 | 供应商联系人 | 必须指定合同对接人和系统账号接收人 | 阻止保存 | Toast "请指定合同对接人" 或 "请指定系统账号接收人" |
| R46 | 等级查询接口 | GET /api/level/query | 返回指定level_type的等级列表 | 供CRM/SRM调用，无权限校验 |
...
| E31 | SCAC条件必填（海运/快递/卡车） | "选择海运/快递/卡车时，此项必填" | 阻断 |
...
|------|--------|---------|---------|---------|
| A01 | 入驻链接生成+过期+使用状态流转 | 手动 | 生成链接→7天后过期→客户点击验证 | AC0a AC0b |
...
| 编号 | 触发点 | 条件/公式 | 输出 | 异常处理 |
|------|--------|----------|------|---------|
| R-F01 | 入驻审核通过 | enterprise_profile.status=30 | 调用货主端 `POST /api/shipper/sub-account/create-root` 创建ROOT子账号+ROOT角色+邮件通知 | 创建失败→异步重试3次→告警运维 |
