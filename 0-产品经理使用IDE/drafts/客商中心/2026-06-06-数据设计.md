...
> **文档版本**: v1.3 | **日期**: 2026-06-06 | **作者**: AI PM（v1.3: 客商中心×货主端数据融合 — 跨模块数据流+接口契约+冻结联动触发器）
...
| 客户关联账户 | — | 纯逻辑实体 | 引用账户模块，客商中心只读展示 |
| 客户账期 | `customer` 内联 | 主表字段 | 与客户1:1 |
| 客户合同 | `customer_contract` | 1:1 子表 | 通过 `customer_id` 关联 |
...
> **设计说明**: 客户主数据表。账期字段内联因为与客户1:1且查询频率高。
...
| `sign_status` | 合同签署状态 | TinyInt | — | Index | 10:未签署, 20:签署中, 30:已签署, 40:已过期 |
| `service_status` | 服务状态 | TinyInt | — | Index | 10:正常, 20:已冻结 |
| `billing_cycle` | 账单周期 | TinyInt | — | — | 10:固定, 20:不固定 |
| `payment_term` | 账期 | TinyInt | — | — | 101:月结, 102:双月结, 103:三月结, 201:周结, 202:到港前结, 203:签收月结, 204:到海外仓结, 205:签收结 |
...
> **设计说明**: V1.1 新增飞书审批流程字段和合同信息回传规则字段。
...
| `company_title` | 我司合同抬头 | String(200) | — | — | 根据域名自动填充 |
| `payment_term` | 签约账期 | TinyInt | — | — | 根据域名和企业属地可选不同值 |
...
| `scac` | SCAC代码 | String(20) | 条件 | — | 明细含"海运/快递/卡车"时必填 |
...
| `expire_time` | 过期时间 | DateTime | Yes | — | 创建时间 + 7天 |
...
erDiagram
    ...
    customer ||--|| customer_contract : "1:1"
...
- **`invitation_link`**: 入驻链接管理表。销售生成链接时创建记录（token=UUID, expire_time=now+7天）。客户点击链接时校验 status=有效且未过期。链接过期后定时任务可将 status 更新为"已过期"。
...
customer_contract.signing_mode = 10 (大陆线上)
  ...
  → is_contract_auto_sync = true → 自动回写 customer.sign_status=已签署 + 日期字段
  → is_contract_auto_sync = false → 销售手工在客户编辑页维护

customer_contract.signing_mode = 20 (香港线下)
  → 上传合同附件 → 保存 → customer.sign_status=已签署
```

### 入驻数据流（新增）

```
销售创建 invitation_link (status=有效, expire_time=now+7天)
  ...
  → 创建 customer 记录 (sign_status=未签署)
  → 在用户管理-待签约中生成签约流程数据
  → 页面置灰 + 显示审核提示
```
...
客商中心（主数据源）
  ...
  ├── customer.sign_status  ──→ 货主端登录准入校验
...
货主端登录 → 校验流程:
  ...
  4. 检查 customer.sign_status ∈ {10,20,30,40}
     → 均可登录（查价不限制）
     → 若 sign_status = 10 (未签署) 且尝试下单 → 提示"请先完成合同签约"
```
...
| sign_status | `customer.sign_status` (10:未签署, 20:签署中, 30:已签署, 40:已过期) | 登录/下单准入 | ✅ 已对齐 |
