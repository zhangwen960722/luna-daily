...
> **文档版本**: v2.4 | **日期**: 2026-08-21 | **作者**: AI PM（v2.4: 签约简易+非标合同分支开始/结束日期自动推导；v2.3: 移除打单服务实体+supplier_print_service表，供应商改左右结构3Tab；v2.2: 签约后账期自动插入规则+billing_cycle/payment_term必填改条件；v2.1: contract_start_date+end_date改为条件必填）
...
> 与 `customer_contract` 结构对齐，一期仅签约核心字段。签约弹窗同客户管理-中国香港签约界面。
