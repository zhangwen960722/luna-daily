...
>
> **签约后账期自动插入**（业务规则）：见 PRD §5.7 签约后账期自动插入规则。
...
**主合同与当前生效判定 / 解约时间与已解约状态**：见 PRD R49（主合同标记）、R50（当前生效主合同判定）、R51（解约时间到期处理）、R52（删除解约时间恢复）与 §7.4 计算公式。
...
>
> **签约后账期自动插入**（业务规则）：见 PRD §5.11.1 供应商签约弹窗步骤3。
...
**设计说明**：等级冻结保护见 PRD R41；查询接口见 PRD R45；权限管控见 PRD §8 权限矩阵。
