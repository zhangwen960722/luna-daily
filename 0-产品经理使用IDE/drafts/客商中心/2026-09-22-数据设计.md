...
>
> **签约后账期自动插入**：签约完成（sign_status=已签署）时系统自动填充账期段——大陆+标准合同：payment_term=签约所选，billing_cycle/settlement_type/settlement_month 级联推导，days_or_date 按所选账期带出（**1–31 的数字**）；大陆+非标准合同 / 中国香港：仅填 contract_company，其余账期字段留空待销售补填。
...
>
> **签约后账期自动插入**：签约完成（sign_status=已签署）时系统自动填充账期段——所属分公司=我司合同抬头（contract_company），账期=签约所选 payment_term，账单周期/结算类型/结算月按级联推导（月结→固定/按月/0；半月结/周结→不固定/按天/空），天数/号按所选账期带出（**1–31 的数字**）。
