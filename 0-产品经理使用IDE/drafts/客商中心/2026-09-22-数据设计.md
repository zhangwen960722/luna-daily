...
> **文档版本**: v2.5 | **日期**: 2026-09-22 | **作者**: AI PM（v2.5: 供应商签约账期枚举收敛为月结/周结/半月结；v2.4: 签约简易+非标合同分支开始/结束日期自动推导；v2.3: 移除打单服务实体+supplier_print_service表，供应商改左右结构3Tab；v2.2: 签约后账期自动插入规则+billing_cycle/payment_term必填改条件；v2.1: contract_start_date+end_date改为条件必填）
> **上游文档**: `2026-09-22-产品文档.md`
...
| 供应商合同 | `supplier_contract` | 1:N 子表 | 通过 `supplier_id` 关联，一个供应商可有多份合同（不同所属公司） |
...
>
> **签约后账期自动插入**（业务规则）：见产品文档 §5.7 签约后账期自动插入规则。
...
| `billing_cycle` | 账单周期 | TinyInt | 条件 | — | 10:固定, 20:不固定；大陆+标准合同签约后系统自动填入，其余场景留空 |
| `payment_term` | 签约账期 | TinyInt | 条件 | — | 101:月结, 102:双月结, 103:三月结, 201:周结, 202:到港前结, 203:签收月结, 204:到海外仓结, 205:签收结；大陆+标准合同签约后系统自动填入，其余场景留空 |
| `settlement_type` | 结算类型 | TinyInt | — | — | 10:按月, 20:按天（由 payment_term 自动推导） |
| `days_or_date` | 天数/号 | Int | — | — | **1–31 的整数**（**只填数字 —— 没有「月末 / 月初」这类文字值**）；按月类账期当「号」用、按天类当「天数」用；大陆+标准合同按所选账期带出，其余场景留空待销售补填；**0 / 空 = 未填**（财务侧视为账期未完善、不允许出账） |
...
| `contract_start_date` | 合同开始日期 | Date | 条件 | — | 非简易合同时必填，默认当天；简易合同由签订日期自动推导 |
| `contract_end_date` | 合同结束日期 | Date | 条件 | — | 非简易合同时必填，自动计算；简易合同=签订日期+期限-1天 |
...
| `sign_date` | 签订日期 | Date | 条件 | — | 简易合同时必填，默认当天 |
...
| `is_new_customer` | 是否新客户 | Boolean | ✅ | — | true:是(新客户), false:否(老客户) |
| `old_customer_code` | 老客户代码 | String(32) | 条件 | — | is_new_customer=false 时必填 |
...
**主合同与当前生效判定 / 解约时间与已解约状态**：见产品文档 R49（主合同标记）、R50（当前生效主合同判定）、R51（解约时间到期处理）、R52（删除解约时间恢复）与 §7.4 计算公式。
...
| `is_peer` | 是否同行 | TinyInt | Yes | — | 10:是, 20:否 |
| `province` | 省 | String(50) | Yes | — | 联系地址-省 |
| `city` | 市 | String(50) | Yes | — | 联系地址-市 |
| `district` | 区 | String(50) | Yes | — | 联系地址-区 |
| `address` | 联系地址 | String(500) | Yes | — | 详细地址（联系地址这一栏必填） |
| `service_status` | 服务状态 | TinyInt | — | Index | 10:正常, 20:已冻结 |
...
### 表10: `supplier_material` | 对应实体: 供应商资料
...
### 表11: `supplier_contract` | 对应实体: 供应商合同 (SupplierContract) — 一期新增

> 与 `customer_contract` 结构对齐，一期为签约核心字段 + 账期字段。签约弹窗同客户管理-中国香港签约界面。
>
> **签约后账期自动插入**（业务规则）：见产品文档 §5.11.1 供应商签约弹窗步骤3。
...
| `tenant_id` | 租户ID | String | Yes | Index | — |
| `supplier_id` | 供应商ID | BigInt | Yes | **FK**, Index | — |
| `contract_company` | 签约主体 | String(100) | Yes | — | 我司合同抬头 |
| `sign_status` | 合同状态 | TinyInt | Yes | Index | 10:未签署, 20:签约中, 30:已签署, 40:已过期, 50:已解约 |
| `is_primary` | 主合同 | Boolean | — | — | 同一供应商仅一份主合同 |
| `company_title` | 我司合同抬头 | String(200) | — | — | 下拉选择，根据域名枚举：飞点→广州飞点/深圳飞点/广东飞点，墨链→墨链 |
| `billing_cycle` | 账单周期 | TinyInt | — | — | 10:固定, 20:不固定；签约后系统自动填入 |
| `payment_term` | 签约账期 | TinyInt | — | — | 101:月结（账单周期=固定）, 104:半月结, 201:周结（账单周期=不固定）；签约时必填 |
| `settlement_type` | 结算类型 | TinyInt | — | — | 10:按月, 20:按天（由 payment_term 自动推导） |
| `settlement_month` | 结算月 | TinyInt | — | — | 0:月结；仅 settlement_type=10 时有效 |
| `days_or_date` | 天数/号 | Int | — | — | **1–31 的整数**（只填数字，没有「月末 / 月初」）；按月类账期当「号」用、按天类当「天数」用；**0 / 空 = 未填** |
| `contract_start_date` | 合同开始日期 | Date | — | — | — |
| `contract_end_date` | 合同结束日期 | Date | — | — | — |
| `termination_date` | 解约时间 | Date | — | — | — |
| `contract_files` | 服务协议附件 | JSON | — | — | 文件列表 |
| `created_at` | 创建时间 | DateTime | Yes | — | — |
| `is_deleted` | 软删除标识 | Boolean | Yes | — | Default: false |
...
**设计说明**：等级冻结保护见产品文档 R41；查询接口见产品文档 R45；权限管控见产品文档 §8 权限矩阵。
...
| `personal_district` | 个人联系地址-区 | String(20) | Yes | — | — |
...
erDiagram
    ...
    supplier ||--o{ supplier_contract : "1:N"
...
- **逻辑删除 `supplier` 时**: 同步软删除 supplier_contact, supplier_material, supplier_contract
...
| customer_level | `customer.customer_level` (SS~G) | **货主端不展示**（客户等级不上货主端）；**后台挂钩**：超级运价三张优惠表（运费优惠 / 泡比优惠 / 附加费优惠）的「客户等级」维度以此为匹配输入，等级变更即改变该客户报价 | ✅ 已对齐 |
...
| pickup_address.zipcode | `pickup_address.zipcode` (邮编) | 货主端取件地址 zip | ✅ 新增（入驻时同步） |
...
| 服务状态 | 10 | NORMAL | 正常 | customer、supplier、level 表的 service_status/status 字段 |
...
| 供应商签约账期 | 101 | MONTHLY | 月结 | supplier_contract 表 payment_term 字段（账单周期=固定） |
| 供应商签约账期 | 104 | SEMI_MONTHLY | 半月结 | supplier_contract 表 payment_term 字段（账单周期=不固定） |
| 供应商签约账期 | 201 | WEEKLY | 周结 | 同上 |
...
| 新增供应商 | POST | /api/supplier | 供应商管理 | 全量字段（基本信息+联系人+资料） | supplierId | 名称重复→Toast |
