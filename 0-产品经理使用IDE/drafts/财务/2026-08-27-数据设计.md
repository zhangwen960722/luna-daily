...
> **版本**：v0.1 | **日期**：2026-08-27 | **作者**：飞点产品
> **覆盖范围**：应收管理 R1-R5 + 资金账户 W1-W6（应付/对账/毛利/开票/成本核算待后续对齐后补充）
> **上游**：[2026-08-27-用户需求.md](2026-08-27-用户需求.md)（RDD v0.3）
...
> 运单收货自动生成，粒度=运单；一条运单按「费用类型 × 币种」可拆多条。账期信息生成时从合同快照锁死。
...
| fee_type | 费用类型 | TinyInt | Yes | — | 10:运费, 20:附加费 |
...
| audit_status | 审计状态 | TinyInt | Yes | Index | 10:待业务审计, 20:待财务审计, 30:已锁定, 40:已驳回, 50:已红冲 |
...
| 字段 | 中文 | 类型 | 必填 | 约束/索引 | 枚举/备注 |
|:---|:---|:---|:---|:---|:---|
| bill_no | 账单号 | String(32) | Yes | **Unique** | 规则：AR+主体+yyyyMM+SEQ |
...
**业务流水 `audit_status`**：
```
待业务审计(10) --业务审计通过--> 待财务审计(20) --财务审计通过--> 已锁定(30)
     └────────────── 驳回(40) ──────────────────┘
已锁定(30) --反审计后红冲--> 已红冲(50)
```
