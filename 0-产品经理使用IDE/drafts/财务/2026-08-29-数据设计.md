...
> **版本**：v0.2 | **日期**：2026-08-29 | **作者**：飞点产品
> **覆盖范围**：应收管理 R1-R5 + 资金账户 W1-W6（应付/对账/毛利/开票/成本核算待后续对齐后补充）
> **上游**：[2026-08-29-用户需求.md](2026-08-29-用户需求.md)（RDD v0.4）
...
| amount | 金额 | Decimal(20,6) | Yes | — | 正=费用，负=优惠 |
...
| payment_terms_type | 签约账期类型 | TinyInt | Yes | — | 月结/双月结/三月结/周结… |
...
| 币种 | `customer_wallet.currency`（除香港富利顿外默认人民币，其他币种财务手工开通） |
...
| account_name | 账户名称 | String(64) | Yes | — | — |
| account_no | 账号 | String(32) | Yes | **Unique** | — |
| bank_name | 开户行 | String(64) | Yes | — | — |
| currency | 账户币种 | String(8) | Yes | — | 单币种账户 |
| is_default | 是否默认 | Boolean | Yes | — | 客户打款默认账户 |
| status | 状态 | TinyInt | Yes | — | 10:启用, 20:停用 |

### 表 6：客户预存款钱包 `customer_wallet`

> 钱包 = 客户 × 主体 × 币种；余额扣减需乐观锁，不可透支。生成节点 = 客户完成「开户 + 签约」后（正常入驻开户完成时 / 特殊入驻签约完成时 / 新合同签约完成时）生成对应主体钱包。
...
| is_default | 是否默认钱包 | Boolean | Yes | — | 入驻自动开人民币默认 |
...
| recharge_id | 关联充值 | BigInt | No | — | 充值/充值退回 |
...
| rate | 汇率 | Decimal(20,6) | Yes | — | 实时基准+手工覆盖 |
