...
> **版本**：v0.3 | **日期**：2026-09-12 | **作者**：飞点产品
> **覆盖范围**：应收管理 R1-R5 + 资金账户 W1-W6（含入账记录）；应付/对账/毛利/开票/成本核算待后续对齐后补充
> **上游**：[2026-09-11-用户需求.md](2026-09-11-用户需求.md)（RDD v1.0 定稿）
>
> ⚠️ **2026-09-12 说明**：本次为**文档维护**——修正一处指向超级运价模块的过期交叉引用（超级运价数据设计已由 `2026-09-11-数据设计.md` 更名为 `2026-09-12-数据设计.md`）。**本文件的字段、表结构、状态机等内容一字未改**，版本仍为 **v0.3**；日期前缀按项目规矩随文件更新前移。
...
> 运单收货自动生成，粒度=运单；一条运单按「费用类型 × 币种」可拆多条。账期信息生成时从合同快照锁死。费用类型引用头程管理 `fee_type` 字典（同方向名称唯一）。
...
| fee_type_id | 费用类型ID | BigInt | Yes | Index | 引用头程管理 fee_type 字典（运费/周六派送费/超重费/地址附加费/商品附加费/报关费/库内贴标费…） |
| currency | 币种 | String(8) | Yes | — | CNY/USD/HKD… |
| amount | 金额 | Decimal(20,6) | Yes | — | 正数=费用金额；优惠不单独成行（见 `discount` 列），负数仅出现在红冲/调整 |
| rate | 汇率 | Decimal(20,6) | Yes | — | 生成/手工创建时自动取实时汇率，人民币=1，不参与金额计算 |
| quantity | 数量 | Decimal(20,6) | No | — | 计费数量（重量/体积/张数）；关联附加费规则按数量计费时，金额=单价×数量自动算 |
| remark | 备注 | String(255) | No | — | 手工备注 |
| biz_time | 业务时间 | DateTime | Yes | — | 业务流水生成时间 |
| source | 数据来源 | String(32) | Yes | — | 报价 / 操作人姓名；只记第一次来源，后续变更不变 |
| publish_price | 公布价 | Decimal(20,6) | Yes | — | 当周公布价 |
| discount | 优惠金额 | Decimal(20,6) | Yes | — | 运费优惠/附加费优惠 |
| receivable_amount | 应收净额 | Decimal(20,6) | Yes | — | publish_price − discount |
...
| bill_id | 关联账单ID | BigInt | No | Index | 生成账单后回填 |
...
> **⚠️ 优惠明细缺口（2026-09-11 记录，一期不修）**：`publish_price` / `discount` / `receivable_amount` 三列中，`discount` 是**汇总值**——系统无法回答"这单为什么减了 3 元、是哪条优惠规则给的、减在哪个费用项上"。
>
> - **优惠规则配置与匹配在超级运价模块**（`drafts/超级运价/2026-09-12-数据设计.md` §3.15 运费优惠 / §3.15b 泡比优惠 / §3.16 附加费优惠），一期**没有落库明细表**（引擎只输出金额，不输出"谁给的"）。
> - **处置**：一期不预建明细表，`discount` 汇总值维持不变；二期**营销模块**落地 `discount_apply_log`（每条优惠 × 每张运单 × 每个费用项一行，含来源规则与券实例）承接明细与优惠券核销，一期历史数据**不回溯拆解**。详见超级运价数据设计 §3.20.4。
> - **影响面**：运单审计的优惠部分无法下钻；客户价格争议需人工比对配置；与货主端 RDD「报价透明度」目标在这部分口径上留缺口（一期接受）。
