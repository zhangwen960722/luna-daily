...
> **上游文档**: `2026-09-22-用户需求.md` (RDD v2.5)
...
| Waybill (运单) | 运单主表（字段全集以运单管理原型 `demo/员工端-demo/Waybill.html` 与财务 PRD §5.1 列表列为准） | 外部实体（本模块不持有、不建表） | 本模块仅登记被其他模块（财务账单明细）引用的运单级补充字段：`fba_no` / `transfer_no` / `product_qty`，见 §2.7 |
...
**业务约束**:
- 附件按注册国家差异化必传的校验规则：见 PRD R02（英国）/ R03（其他欧洲国家）
...
**业务约束**:
- 名称唯一 / 话术模板条件必填的校验规则：见 PRD R11 / R12 / R13
...
> **本节只登记被其他模块（财务「账单管理 → 账单详情」明细表）引用的运单级补充字段**；**运单主表字段全集以运单管理原型 `demo/员工端-demo/Waybill.html` 与财务 PRD §5.1 列表列为准**，本节不重复登记、不代表运单主表全量设计。
