...
> **文档版本**: v2.7 | **日期**: 2026-09-24 | **作者**: 飞点跨境产品经理
> **上游文档**: `2026-09-24-产品文档.md`（v2.6）
> **本次更新**: 费用类型「费用方向」由单选改多选（应收 / 应付 / 销售成本），`fee_direction` 改多值 + 唯一约束改为名称唯一；新增 `sync_sales_cost_for_peer`（同行客户同步销售成本）字段
...
| FeeType (费用类型) | `fee_type` | 独立表 | 费用科目字典，费用名称唯一，方向多选（应收/应付/销售成本） |
...
| Waybill (运单) | 运单主表（字段全集以运单管理原型 `demo/员工端-demo/Waybill.html` 与财务产品文档 §5.1 列表列为准） | 外部实体（本模块不持有、不建表） | 本模块仅登记被其他模块（财务账单明细）引用的运单级补充字段：`fba_no` / `transfer_no` / `product_qty`，见 §2.7 |
| 提单配载明细 (BLLoad) | 待头程管理专项落表（原型 `舱位管理.html` 已承载「提单号 + 配载信息」） | 外部关联（供财务提单平摊取数） | 提单（舱位）→ 运单的装柜配载关系 + 实收体积，供财务「提单平摊应付」按实收体积平摊取数，见 §2.8 |
...
**业务约束**:
- 附件按注册国家差异化必传的校验规则：见产品文档 R02（英国）/ R03（其他欧洲国家）

---

### 2.3 表: `fee_type` | 对应实体: FeeType

> **设计说明**: 费用类型字典表，供下游运单计费模块引用。费用名称唯一（租户内）；费用方向为多选（应收/应付/销售成本），同一费用科目可同时属于多个方向。无权限管控。提供查询接口供运单计费调用。

| 字段名 (En) | 字段名 (Cn) | 类型 (Type) | 必填 | 约束/索引 | 枚举/备注 |
|:---|:---|:---|:---|:---|:---|
| `id` | 主键 | BigInt | Yes | **PK** | 雪花ID |
| `tenant_id` | 租户ID | String | Yes | Index | SaaS数据隔离 |
| `fee_name` | 费用名称 | String(100) | Yes | **部分唯一**（tenant_id, fee_name, status=正常） | 同名仅一个启用，冻结的同名可多个；如"周六派送费""超重费" |
| `fee_direction` | 费用方向 | Varchar(50) | Yes | — | 多选，逗号分隔方向码：10 应收 / 20 应付 / 30 销售成本（如 "10,30" = 应收+销售成本） |
| `sync_sales_cost_for_peer` | 同行客户同步销售成本 | TinyInt | No | — | 仅当 `fee_direction` 含「30 销售成本」时生效；10 同步 / 20 不同步（**默认 20**）。同行客户（`customer.is_peer=10`）应收流水是否复制销售成本流水的开关，见财务产品文档 R5.10 |
| `is_system` | 是否系统预置 | Boolean | Yes | — | 系统预置的费用类型（如「运费」）不可删除/冻结；Default: false |
...
**查询接口**:
- 供运单计费模块调用：按 fee_direction（费用类型方向包含所选方向）/ status 过滤，返回费用类型列表
...
**业务约束**:
- 名称唯一 / 话术模板条件必填的校验规则：见产品文档 R11 / R12 / R13
...
### 2.7 运单级补充字段（供财务账单明细引用）

> **本节只登记被其他模块（财务「账单管理 → 账单详情」明细表）引用的运单级补充字段**；**运单主表字段全集以运单管理原型 `demo/员工端-demo/Waybill.html` 与财务产品文档 §5.1 列表列为准**，本节不重复登记、不代表运单主表全量设计。
> 本模块不持有运单数据（见 §4.4 纯逻辑实体说明），下表字段**挂在运单（Waybill）实体上、随运单主表落库**，头程管理侧不新增表，仅登记字段口径供财务账单明细引用。

**箱级 vs 运单级（务必区分，研发不得混用）**

运单管理原型 `demo/员工端-demo/Waybill.html` L3557-3562 的「编辑快递单」弹窗是**按货箱**维护的，列为：**系统箱号 `sysNo` / 扩展箱号 `extNo` / 承运商代码 / 转单号 `transferNo`** —— 一个运单多个货箱则多行，每行描述一个箱。本节登记的是**运单级**字段（一个运单一条值）：

| 对比项 | 箱级（原型「编辑快递单」弹窗） | 运单级（本节登记，供财务账单明细引用） |
|:---|:---|:---|
| 字段 | `sysNo` 系统箱号 / `extNo` 扩展箱号 / 承运商代码 / `transferNo` 转单号 | `fba_no` FBA 单号 / `transfer_no` 转单号 / `product_qty` 产品数量 |
| 粒度 | 1 运单 : N 货箱，逐箱填写 | 1 运单 : 1 值（1:1） |
| 业务含义 | 该**货箱**自身的箱号、承运商与转单号 | 该**运单**关联的快递主单号、承运商转单号、商品数量合计 |
| 维护与引用 | 运单管理原型箱级维护与展示 | 财务「账单管理 → 账单详情」明细表按列引用 |

> **同名不同义提醒**：`transfer_no`（运单级「转单号」）与原型箱级 `transferNo`（箱级「转单号」）**不是同一个字段** —— 两者并存、语义不同，不得合并取值或以箱级值直接填充运单级。`fba_no`（FBA 单号）是**运单级**字段，与箱级字段无关。

| 字段名 (En) | 字段名 (Cn) | 类型 (Type) | 必填 | 约束/索引 | 枚举/备注 |
|:---|:---|:---|:---|:---|:---|
| `fba_no` | **FBA 单号** | String(32) | No | Index | **运单级**。该运单的 **FBA 货件编号**（如 `FBA15XXXXXXX`），来源 = 下单时录入 / FBA 货件回传；**非 FBA 运单留空**。一个运单一个（**多货件场景一期取主货件，取值口径待定**） |
| `transfer_no` | 转单号 | String(64) | No | Index | **运单级**，承运商转单号。与原型箱级「转单号」`transferNo` **不是同一个字段**。**多箱运单的取值口径待定（建议取主箱 / 首箱）** |
| `product_qty` | 产品数量 | Int | No | — | **派生字段** = 该运单商品明细各行「数量」之和（原型 `Waybill.html` 商品明细列 `qty` / `quantity`）。**不落库存储**，财务账单明细取数时按运单**实时汇总** |

**取值与实现约定**:

- `fba_no` / `transfer_no`：运单创建或下单回传时写入运单主表，箱级字段（`sysNo` / `extNo` / 承运商代码 / `transferNo`）继续按箱维护，两套数据并存
- `product_qty`：**采用「取数时实时汇总、不落库」方案** —— 财务账单明细查询时按运单汇总商品明细行数量，避免运单商品明细变更后数量快照失真
- 财务账单明细展示这三列时，取值一律取**运单级**字段；箱级字段不参与账单明细取值

**关联关系**:
- `Many-to-One` with Waybill（运单主表，本模块不建表；字段随运单实体落库）

---

### 2.8 提单配载明细（供财务提单平摊取数）

> **本节登记「提单（舱位）→ 运单」的装柜配载关系与实收体积**，供财务「提单平摊应付」按实收体积平摊取数（见财务数据设计「提单平摊应付」节）。**提单（舱位）实体与配载明细的完整建模待头程管理专项**（舱位管理原型 `舱位管理.html` 已承载「提单号 + 配载信息」，数据设计尚未落表），本节先固定平摊取数所需的字段口径。

| 字段名 (En) | 字段名 (Cn) | 类型 | 必填 | 约束/索引 | 枚举/备注 |
|:---|:---|:---|:---|:---|:---|
| `bl_no` | 提单号 | String(64) | Yes | Index | 提单（舱位）唯一标识；财务 `ar_flow.bl_amortize` 引用 |
| `waybill_no` | 运单号 | String(32) | Yes | Index | 装进该提单的运单 |
| `received_volume` | 实收体积 | Decimal(20,6) | Yes | — | **平摊分子**；舱位管理配载信息「体积 下单/实收」的**实收**值（单位 CBM / m³） |
| `box_no` | 货箱号 | String(32) | No | — | 货箱维度配载时的箱号（运单维度聚合时为空） |

**取数与平摊口径**：

- **分母** = 提单总实收体积 = Σ 该提单下各运单 `received_volume`；**分子** = 各运单 `received_volume`；
- 财务平摊按「各运单实收体积 ÷ 提单总实收体积」分摊提单级应付总额，**尾差挂最后一个运单**（见财务数据设计「提单平摊应付」节）。
...
erDiagram
    ...
    FEE_TYPE {
        ...
        String tenant_id
        String fee_name "唯一"
        String fee_direction "多选 10应收/20应付/30销售成本"
        TinyInt status "10:正常 20:已冻结"
        String operator
    }
...
| FeeDirection | 30 | SALES_COST | 销售成本 | FeeType |
...
| 查费用类型列表 | GET | /api/fee-types | 费用类型_主列表 | feeName, feeDirection(多选数组), page, pageSize | 分页列表 | Toast |
| 新增费用类型 | POST | /api/fee-types | 弹窗 | feeName, feeDirection(多选数组) | 实体 | Toast |
| 编辑费用类型 | PUT | /api/fee-types/{id} | 弹窗 | feeName, feeDirection(多选数组) | 实体 | Toast |
| 切换费用类型状态 | PATCH | /api/fee-types/{id}/toggle-status | 列表行 | — | 新状态 | Toast |
| 查费用类型（下游） | GET | /api/fee-types/query | 运单计费 | feeDirection(方向包含匹配), status | 费用类型列表 | Toast |
