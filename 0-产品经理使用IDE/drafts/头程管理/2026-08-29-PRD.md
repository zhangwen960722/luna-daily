...
- 用户需求(RDD)：`drafts/头程管理/2026-08-29-用户需求.md` (v2.2)
- 数据设计：`drafts/头程管理/2026-08-29-数据设计.md` (v2.2)
- 需求背景：待补充
- 原型：待更新
...
| is_credit | 是否赔付类型 | 开关 | 否 | — | 标记为赔付类型，供财务入账记录捕捉；仅应收方向显示 |
...
| 运单费用 -- 审计 / 反审计 | **不在运单管理页**；入口统一为「财务中心 → 运单审计」（`财务_运单审计.html`）：运单级 = 列表勾选运单 + 选费用方向（整票）；流水级 = 点运单号进详情后在应收 / 应付 Tab 勾选流水。**反审计仅财务角色可执行** | 否 |
...
- 术语表：
  ...
  - **运单 (Waybill)**：物流运输单据
- 原型链接：见 0.2 关联链接
- 用户需求(RDD)（完整业务流程+字段表）：`drafts/头程管理/2026-08-29-用户需求.md`
- 数据设计（完整 Schema+ER 图）：`drafts/头程管理/2026-08-29-数据设计.md`
