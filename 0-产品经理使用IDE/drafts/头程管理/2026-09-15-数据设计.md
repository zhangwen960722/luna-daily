...
> **文档版本**: v2.3 | **日期**: 2026-09-15 | **作者**: 飞点跨境产品经理
> **上游文档**: `2026-09-15-用户需求.md` (RDD v2.3)
> **本次更新**: 登记运单级补充字段「**FBA 单号** / 转单号 / 产品数量」（供财务账单明细引用），并显式区分箱级与运单级字段
...
| `id` | 主键 | BigInt | Yes | **PK** | 雪花ID |
| `tenant_id` | 租户ID | String | Yes | Index | SaaS数据隔离 |
| `fee_name` | 费用名称 | String(100) | Yes | **部分唯一**（tenant_id, fee_name, fee_direction, status=正常） | 同方向同名仅一个启用，冻结的同名可多个；如"周六派送费""超重费" |
...
| `is_credit` | 是否赔付类型 | Boolean | Yes | — | 标记为赔付类型，供财务**识别赔付（抵扣）行**；仅应收方向；Default: false |
| `credit_type` | 赔付类型 | String(32) | No | — | is_credit=true 时必填；引用系统枚举配置（赔付类型，可自定义），如超时赔付/丢件赔付/货损赔偿 |
