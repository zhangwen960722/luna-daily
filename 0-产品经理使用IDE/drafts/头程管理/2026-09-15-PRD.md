...
- 用户需求(RDD)：`drafts/头程管理/2026-09-15-用户需求.md` (v2.3)
- 数据设计：`drafts/头程管理/2026-09-15-数据设计.md` (v2.3)
- 需求背景：待补充
- 原型：待更新
...
| credit_type | 赔付类型 | 标签 | 超时赔付/丢件赔付/货损赔偿=warning橙色 |
...
| is_credit | 是否赔付类型 | 开关 | 否 | — | 标记为赔付类型，供财务**识别赔付（抵扣）行**；仅应收方向显示 |
| credit_type | 赔付类型 | 下拉选择 | 条件必填 | is_credit=是时必选 | 超时赔付/丢件赔付/货损赔偿 |
...
- [保存校验]：新增时校验"同方向同名仅一个启用"——相同费用方向下已有启用的同名则禁止新增（需先冻结旧的）；启用已冻结的同名时若已有启用的同名则弹窗提示

**关联接口**：
- 查询：`GET /api/fee-types` (params: feeName, feeDirection, page, pageSize)
- 新增：`POST /api/fee-types` (body: feeName, feeDirection, isCredit, creditType)
- 编辑：`PUT /api/fee-types/{id}` (body: feeName, feeDirection, isCredit, creditType)
...
| E08 | 费用类型 -- 同方向同名已有启用 | "该费用方向下已存在启用的同名费用类型" | 阻断 |
...
- 术语表：
  ...
  - **运单 (Waybill)**：物流运输单据
- 原型链接：见 0.2 关联链接
- 用户需求(RDD)（完整业务流程+字段表）：`drafts/头程管理/2026-09-15-用户需求.md`
- 数据设计（完整 Schema+ER 图）：`drafts/头程管理/2026-09-15-数据设计.md`
