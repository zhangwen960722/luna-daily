...
- 用户需求(RDD)：`drafts/头程管理/2026-09-22-用户需求.md` (v2.5)
- 数据设计：`drafts/头程管理/2026-09-22-数据设计.md` (v2.6)
- 需求背景：待补充
- 原型：待更新
...
- In Scope（本期 P0）：VAT管理（货主端+员工端，含国家差异化附件）、费用类型管理（名称唯一+方向多选）、物流事件管理（名称唯一+Ship Track绑定+查询接口）、亚马逊物流事件管理（状态启停+查询接口）、标识管理（查询接口供舱单/运单打标）
...
**搜索条件**：费用名称（文本输入）、费用方向（下拉多选：应收/应付/销售成本）
...
|--------|--------|------|------|
| fee_name | 费用名称 | 文本 | — |
| fee_direction | 费用方向 | 标签 | 多选：应收=success绿色, 应付=warning橙色, 销售成本=danger红色 |
| is_system | 是否系统预置 | 标签 | 系统预置=info灰色 |
...
| 字段名 | 中文名 | 类型 | 必填 | 校验规则 | 备注 |
|--------|--------|------|------|---------|------|
| fee_name | 费用名称 | 文本输入 | 是 | 非空 + 名称唯一 | — |
| fee_direction | 费用方向 | 多选下拉 | 是 | 至少选一个 | 应收 / 应付 / 销售成本 |

**交互行为**：
- [查询]：支持按费用名称+费用方向组合搜索
- [重置]：清空搜索条件
- [新增]：弹窗"新增费用类型"，费用名称必填 + 费用方向多选（至少选一个）
- [查看]：点击行"查看" → 弹窗"查看费用类型"，所有字段只读（费用类型创建后不可编辑），隐藏"确定"按钮，仅显示"关闭"
- [冻结/启用]：二次确认弹窗 `ElMessageBox.confirm`（type: warning），确认后切换状态并Toast提示
- [系统预置]：系统预置的费用类型（如「运费」，is_system=true）不显示冻结/删除操作，仅可查看
- [保存校验]：新增时校验"同名仅一个启用"——已有启用的同名则禁止新增（需先冻结旧的）；启用已冻结的同名时若已有启用的同名则弹窗提示

**关联接口**：
- 查询：`GET /api/fee-types` (params: feeName, feeDirection(多选数组), page, pageSize)
- 新增：`POST /api/fee-types` (body: feeName, feeDirection(多选数组，如 ["应收","销售成本"]))
- 编辑：`PUT /api/fee-types/{id}` (body: feeName, feeDirection(多选数组))
...
### 5.9 舱位管理（提单）· 财务信息 Tab

**页面信息**

- 路径：头程管理 / 舱单管理 / 舱位管理 → 提单详情 → 「财务信息」Tab
- 类型：提单级应付费用管理（加应付费用 + 批量平摊到运单）
- 访问角色：财务专员
- **定位**：应付业务流水「提单平摊」来源的**提单侧录入入口**（见财务 PRD R76 / 财务数据设计「提单平摊应付」节）—— 财务在提单上记一笔提单级应付费用（待摊），再批量平摊到该提单下各运单。**本 Tab 同时是提单侧应付流水的只读镜像**：除提单级费用（待摊 / 已摊）外，还展示运单审计「添加应付」挂靠同步到本提单的应付流水（`source_type=20`，提单侧只读，改 / 删回运单侧）。

**Tab 结构**：

- 顶部：**[添加应付费用] [批量平摊] [批量撤销平摊]** 按钮 + 提单审计状态 tag + 该提单「总费用」合计（**提单级审计 / 反审计统一在舱位主列表**，本 Tab 顶部不再放提单级审计按钮）。
- 费用明细表：勾选列 / 费用类型 / 金额（原币）/ 金额（本币）/ 币种 / 汇率 / 供应商 / OA审批编号 / 备注 / 业务时间 / 平摊状态（待摊 / 已摊）/ **审计状态** / 账单编号 / 创建时间 / 创建人 / 操作（查看 / 平摊 / 撤销平摊 / 删除）；顶部 **[审计] [反审计]**（**流水级**：勾选已摊行 → 单条审计 / 反审计，只动该条 `audit_status`、不动提单级标记）。**审计状态列只对提单审计对象（已摊行）展示两态，其余显示「—」**：① **待摊行**（`source_type=30` + 待摊）——审计状态「—」、**不可勾选审计**，操作列 平摊 / 删除；② **已摊行**（`source_type=30` + 已摊）——审计状态「待审计 / 已审计」、**可勾选审计 / 反审计**（反审计前置 = 未关联账单），操作列 撤销平摊 / 删除（已审计 → 置灰）；③ **运单挂靠行**（`source_type=20`）——审计状态「—」、提单侧**只读不审**（归运单审计，由「平摊状态」列「运单挂靠」区分），操作列仅「查看」（改 / 删回运单侧）。

**添加应付费用弹窗**：提单号（默认当前提单，只读）/ 业务时间（必填）/ OA审批编号（必填）/ 供应商（必填）/ 费用类型（必填）/ 币种（必填）/ 汇率（必填）/ 金额（原币，必填）/ 备注 / **是否自动平摊（开关，默认勾选）** —— 勾选则保存即按实收体积自动平摊到该提单下各运单（直接生成已摊行）；不勾选则落 `ar_flow` **待摊行**（`flow_category=20`、`amortize_status=待摊`、`bl_amortize` = 提单号、`source_type=30`、审计状态 = 空），之后手动批量平摊。

**费用类型唯一性**：本提单下费用类型唯一（提单维度）；「添加应付费用」新增时若本提单已有同费用类型 → 阻断；「自动平摊 / 批量平摊」时若任一目标运单已存在该费用类型（运单维度）→ 平摊失败、提单侧新增一并取消（整单不落）。见财务 PRD R76。

**批量平摊**：对待摊行点「平摊」→ 按「各运单实收体积 ÷ 提单总实收体积」把待摊总额摊到该提单下各运单（实收体积取自本页「配载信息」；**尾差挂最后一个运单、该条描述备注「含尾差」**）→ 生成 N 条运单应付流水，待摊行转「已摊」。**平摊是原子操作**：目标运单中任一「应付」已整票审计 → 本次平摊整个阻断（见财务 PRD R76）。

**反分摊**：对已摊行点「反分摊」→ 该提单下「该流水未审计（流水级）+ 未关联账单 + 目标运单「应付」未整票审计」的平摊流水批量删除、待摊行退回「待摊」改金额重摊（见财务 PRD R76）。

**提单审计 / 反审计（两层）**：**提单级**在舱位主列表（勾选提单 →「审计 / 反审计」→ 选「审计范围：应付」，整提单全部已摊应付流水统一审计 / 反审计）；**流水级**在财务信息 Tab 明细表（勾选已摊行 →「审计 / 反审计」，单条生效、只动该条 `audit_status`）。**审计归属、两层审计、平摊 / 撤销平摊三道前置、挂靠编辑权限、成账口径的完整规则见财务 PRD R76**。

**关联接口**：财务「提单平摊」接口（`POST /api/finance/ar-flows/amortize` 等，见财务 PRD §5.15）。
...
| R10 | 费用类型新增/编辑 | fee_name 唯一（租户内全局唯一，方向为多选属性） | 保存成功 | 重复时提示"费用名称已存在" |
...
| 运单费用 -- 查看 | 是 | 是（仅本企业，只读） |
| 运单费用 -- 新增/编辑/删除 | 是（受**运单状态 + 运单审计标记（按应收/应付方向）+ 账单关联 + 流水审计标记**约束，见财务 PRD R61）：**运单该方向已整票审计 → 整票不允许新增该方向费用**；运单未审计时，**已审计的单条流水不可变更、但可新增流水**。**入口在「财务中心 → 运单审计」**（`财务_运单审计.html`）—— **运单管理页不提供费用新增 / 编辑 / 删除**（该页只做费用查看与整票审计） | 否 |
| 运单费用 -- 审计 / 反审计 | **审计 = 是**：运单管理页（`Waybill.html`）**列表勾选运单 → 工具栏「运单审计」→ 选费用方向（应收 / 应付）→ 该方向全部流水统一审计**（运单级整票）。**反审计 = 否**（仅财务角色）：入口在「财务中心 → 运单审计」（`财务_运单审计.html`），运单级整票与流水级勾选都在那一页 | 否 |
...
| E08 | 费用类型 -- 同名已有启用 | "已存在启用的同名费用类型" | 阻断 |
| E09 | 费用类型 -- 费用名称/方向为空 | "请输入费用名称" / "请选择费用方向"（方向至少选一个） | 阻断 |
...
| A09 | 费用类型名称唯一校验 | 手动 | 同名+启用→阻断 | AC03-FT02 |
...
- 术语表：
  ...
  - **运单 (Waybill)**：物流运输单据
- 原型链接：见 0.2 关联链接
- 用户需求(RDD)（完整业务流程+字段表）：`drafts/头程管理/2026-09-22-用户需求.md`
- 数据设计（完整 Schema+ER 图）：`drafts/头程管理/2026-09-22-数据设计.md`
