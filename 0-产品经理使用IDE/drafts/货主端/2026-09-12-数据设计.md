...
> 报价接口的字段过滤口径（`caller=shipper` 只返回公布价+优惠、不下发成本底价/利润/盈亏标识）与未登录拦截见 PRD §6 R06e。
...
> 货主端**不配置、不落库**任何优惠规则，只在查价时按当前登录客户匹配并展示。三张优惠表（`freight_discount` / `density_discount` / `surcharge_discount`）与父实体（服务组合 / 附加费规则）的归属见 §1 实体 x 表映射 与 §3 ER 图。匹配优先级（客户特批 > 客户等级）、优惠值带符号口径、最终报价公式、超重超长参与方式等**业务规则与计算公式见 PRD §6（R06/R06a–d、R21–R23、R08a）与 §7.2**。
...
> 登录校验链（本地认证 → 子账号状态 → 跨模块企业状态 → 签发 JWT）见 PRD §13.4。

### 6.5 失败降级策略

> 跨模块接口的失败降级策略（create-root 重试、batch-freeze/enable 失败告警、客户状态超时降级、报价接口超时不降级返回缓存价）见 PRD §13（R-F01 / §13.4 / §13.5）。
...
> 跨模块口径不一致的处理结论（超重超长参与方式、接口路径、优惠金额展示顺序、泡比优惠抵扣约束、待建页面、实体归属等）见 RDD §12.5 不一致处理标注 / §12.6 跨模块口径结论 与 PRD §14 跨模块口径结论。本数据设计仅保留字段级口径（见 §2 Schema）。
