...
> Phase 2：方案架构 | 基于 RDD v1.3 | 2026-09-12（v1.3: 整体刷新——对齐现行原型（12 页）与「超级运价」2026-09-12 定稿的优惠三表口径：优惠对象类型一期同时开放客户等级/客户特批、泡比优惠进入 Phase 1、超重超长拣货触发）
> 上一版：v1.2（2026-06-06，货主端×客商中心数据融合 — 跨模块接口契约+登录校验链+同步机制）
...
| -- | **泡比优惠规则** | `density_discount` | 引用 | **★ 变更（2026-09-12）**：超级运价模块，**一期启用**（原「二期」作废），查价时关联 |
...
> **★ 变更（2026-09-12）**：新增 `density_discount`（泡比优惠）与 `sailing_schedule`（船期）两条引用关系。货主端**全部为只读引用**，不落库运价侧任何表。
...
| `email` | 邮箱 | String(128) | **No** | -- | **非必填**（与原型一致：新增子账号必填项仅 账号/姓/名/性别/手机号）。~~原「待确认 D1」~~ **★ 已定（2026-09-12）**：货主端手动创建的子账号邮箱非必填；客商中心侧「客户联系人邮箱」必填，二者不矛盾——ROOT 账号邮箱来自客户联系人（`is_account_receiver=true`，必填），手动创建的子账号邮箱非必填 |
...
> **⚠️ 原型与本文档的差异（如实记录，待产品确认）**：现行原型 `子账号管理.html` 新增/编辑弹窗的必填标识为 **账号 / 姓 / 名 / 性别 / 手机号**（邮箱无必填星标），与上表一致；但原型列表中**没有「修改密码」按钮**、也**没有 `is_root` 过滤与「分配资源」下拉中的 ROOT 角色选项**（原型下拉为「财务角色/业务角色/客服角色」三个演示值）。上表 `is_root`、`password_hash` 为文档侧设计口径，原型待补齐。
...
> **★ 变更（2026-09-12）**：结果行字段按超级运价 PRD §5.13「货主端在线询价结果表列」口径重排——新增 `service_combination_code` / `delivery_regions` / `freight_discount_detail` / `density_discount_detail` / `surcharge_discount_detail`，`final_price` 的计算口径同步为 ~~「公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠」~~。**不再存在「优惠后」中间列**。
>
> **★ 变更（2026-09-12）：优惠值正负号翻转（覆盖旧约定）** —— 三条优惠明细中的金额字段一律为**带符号优惠值**：**负数 = 减钱（正向优惠）**、**正数 = 加价（反向优惠）**（~~原「正数=减价、负数=加价」~~ 作废）；`final_price` 口径改写为 **`(公布价 + 运费优惠值 + 泡比优惠值) + (附加费合计 + 附加费优惠值合计)`**（~~上一版 `公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠`~~ 作废——优惠值本身为负数，负数值即等于原来的减项）；附加费优惠的固定减免上限由 ~~`优惠值 ≤ 应收单价`~~ 改为 **`|优惠值| ≤ 应收单价`**。**折扣率口径不变**（乘数，`金额 × 率`）。
...
| `final_price` | 最终报价 | Decimal(18,2) | Yes | -- | ~~公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠~~ **★ 变更（2026-09-12）**：`(公布价 + 运费优惠值 + 泡比优惠值) + (附加费合计 + 附加费优惠值合计)` |
...
> **★ 变更（2026-09-12）**：补充三张优惠表与父实体（服务组合 / 附加费规则）及客户主体的匹配关系；三张表均在**超级运价模块**，货主端只读匹配。
...
- `price_inquiry_result.freight_discount_detail` / `density_discount_detail` / `surcharge_discount_detail`：**★ 变更（2026-09-12）** 三张优惠表各自的命中规则名 + **优惠值（带符号：负数=减钱、正数=加价）**，用 JSON 快照，便于追溯"当时按哪条规则算的"
...
- **泡比优惠：查询 `density_discount`**（**★ 变更（2026-09-12）**：一期启用，原「二期」作废）
- 船期/截单：查询 `sailing_schedule`（运输详情弹窗的当周船期×各交货地截单矩阵）
- 以上均为**只读查询**，货主端不写入运价相关表

> **★ 变更（2026-09-12）** 三层询价数据安全（超级运价 PRD R27）：同一报价 API `POST /api/freight-rate/query`，请求体 `caller=shipper` 时**只返回公布价+优惠，不返回成本底价/利润/盈亏标识**；未登录 + `caller=shipper` → 401。
...
> **★ 变更（2026-09-12）**：页面名对齐现行原型——`收货地址管理` → **地址管理**；补充 VAT资料新增、账单管理、资金账户、后台任务四行。**★ 2026-09-12 产品确认**：账单管理 / 资金账户 **归「财务」模块**（实体见 `drafts/财务/2026-09-12-数据设计.md`）、后台任务 **归「后台任务」模块**（见 `drafts/后台任务/`）——均按「**引用其他模块页面**」处理，**货主端不定义实体**，上表按引用登记即可（D8 已关闭）。原"待确认后回填"说明作废，此前表述为源表。
...
- 企业管理员可为子账号修改密码：`PUT /api/shipper/sub-account/{id}/password`
  - **⚠️ 原型现状（2026-09-12 核对）**：`子账号管理.html` 列表操作列**当前只有 编辑 / 分配资源 / 冻结(启用)**，**没有「修改密码」按钮**——本项为文档侧设计口径，原型待补齐
...
> **⚠️ 原型现状（2026-09-12 核对）**：`子账号管理.html` / `角色管理.html` 的演示数据中**未体现 `is_root` 过滤、ROOT 角色保护、资源范围上限**；分配资源下拉为「财务角色/业务角色/客服角色」三个演示值。本节为后端设计口径，原型待补齐。
...
> **★ 变更（2026-09-12）新增**：本节对齐「超级运价」2026-09-12 定稿口径。货主端**不配置、不落库**任何优惠规则，只在查价时按当前登录客户匹配并展示。
>
> **★ 变更（2026-09-12）：优惠值正负号翻转（覆盖旧约定，本节伪代码已同步）** —— 固定减免的**优惠值本身带符号**：**负数 = 减钱（正向优惠）**、**正数 = 加价（反向优惠）**（~~原「正数=减价、负数=加价」~~ 作废）；计算由 ~~`原金额 − 优惠值`~~ 改为 **`结果金额 = 原金额 + 优惠值`**；显示：负 → `-¥X`、正 → `+¥X`、0 → 「全免」；附加费优惠的固定减免上限由 ~~`优惠值 ≤ 应收单价`~~ 改为 **`|优惠值| ≤ 应收单价`**（绝对值比较）。**折扣率口径不变**：乘数语义（`<1` 折扣、`=1` 原价、`>1` 加价；一期 `0.01 ≤ 率 ≤ 1` 不支持加价），计算仍为 **`金额 × 率`**。
...
```
# 0. 优惠值符号约定（★ 2026-09-12 翻转，覆盖旧约定）
#    固定减免 discount_value：负数 = 减钱（正向优惠）；正数 = 加价（反向优惠）
#    结果金额 = 原金额 + discount_value        ← 旧口径「原金额 − discount_value」作废
#    折扣率 rate 为乘数：<1 折扣 / =1 原价 / >1 加价（一期 0.01 ≤ rate ≤ 1，不支持加价）
#    折扣率计算 = 金额 × rate（口径不变，不受符号翻转影响）
...
对每条命中的附加费规则 r：
  ...
  附加费优惠后单价 = r.应收单价 + 附加费优惠值              # ★ 旧口径「应收单价 − 优惠值」作废
  # 折扣率时：附加费优惠后单价 = r.应收单价 × 附加费优惠.rate（乘数，口径不变）

# 4. 最终报价（★ 2026-09-12 符号翻转后重写；超级运价 PRD §7.4 的「折扣后运费 + 优惠后附加费」与此等价）
...
约束：
  ...
  b. 附加费优惠的固定减免上限：|附加费优惠值| ≤ 应收单价（旧口径「优惠值 ≤ 应收单价」作废）
  c. 泡比优惠只作用于运费侧，不得抵扣附加费
```

> **⚠️ 未决项（见 §8 不一致处理标注）**：
> - **★ E9 已定（2026-09-12）——超重超长在查价阶段的参与方式**：**★ E9 已定（2026-09-12，产品确认）**：查价阶段**能否**算出「超重超长」**取决于包裹信息的录入方式**，不是"算不算"的问题 ——① **按单箱录入**（填了长/宽/高）→ **可以算**：超重超长按公式判定（该规则计价单位**仅按箱**）、**体积重 = 长×宽×高 ÷ 计泡系数** → **在查价结果中作预估展示**；② **按总体重量与体积录入**（无单箱长宽高）→ **算不出，不展示该项**。**正式计费仍以「拣货实测」为准**（客户自填尺寸仅供预估），故结果区提示条**保留**并继续声明"以拣货实测为准"；**规则口径不变**：「字段&公式」型规则的**正式触发与费用流水生成仍在「拣货」节点**，查价阶段只是**预估展示、不落库、不进账单**。
> - **附加费优惠与泡比优惠的叠加顺序**：超级运价 PRD §7.4 的公式为 `最终报价 = 折扣后运费 + 优惠后附加费`，而原型费用明细弹窗展示为 ~~`公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠`~~。**★ 变更（2026-09-12，符号翻转）**：货主端口径统一为 **`(公布价 + 运费优惠值 + 泡比优惠值) + (附加费合计 + 附加费优惠值合计)`**，与超级运价 §7.4 的「折扣后运费 + 优惠后附加费」**完全等价**（`折扣后运费` 即 `公布价 + 运费优惠值 + 泡比优惠值`）；原「两种写法靠减法交换律等价」的说法在符号翻转后**不再成立为"两种写法"**，全系统同一写成法，已在 §8 更新。
...
| **ValidityType** | 30 | SINGLE_USE | 单次有效（二期，不作废但一期不开放） | 三张优惠表 |
...
> **★ 变更（2026-09-12）**：新增 `DiscountTargetType` / `DiscountMode` / `ValidityType` / `FeeDirection` 四组枚举。要点：① `DiscountTargetType` **一期同时开放 10 与 20**（原「一期仅 20 客户等级」作废）；② `DiscountMode` 一期仅 20；③ `ValidityType` 一期 10/20；④ `FeeDirection` 固定 10。⑤ **★ 2026-09-12 追加**：`DiscountMode` 的**金额语义**明确——`10 RATE` 为**乘数**（`金额 × 率`；`<1` 折扣、`=1` 原价、`>1` 加价；一期 `0.01 ≤ 率 ≤ 1` 不支持加价，**不变**）；`20 FIXED` 为**带符号调整量**（**负数=减钱、正数=加价**；`金额 + 优惠值`；显示负 `-¥X` / 正 `+¥X` / 0「全免」；上限按 `|优惠值| ≤ 应收单价` 校验）。原口径「固定减免正数=减价、负数=加价，计算 `金额 − 优惠值`」**作废**。
...
客商中心（System of Record）
  customer                          ← 客户主数据唯一源
    ...
    └── customer_level (SS~G)       → ★ 变更（2026-09-12）：不再只是"展示"，而是
                                       三张优惠表「客户等级」匹配的输入（客户特批 > 客户等级）
...
> **★ 变更（2026-09-12）**：`customer_level` 的用途从「暂不控制权限」升级为**三张优惠表「客户等级」维度的匹配输入**（原 v1.2 的「未来如需按等级差异化定价，再补充」已落地）。
...
货主端处理:
  ...
  2. 创建 ROOT 子账号:
     ...
     -- ★ 变更（2026-09-12）：contact_email 来自客商中心「客户联系人.邮箱」，
     --   该字段在客商中心侧必填（is_account_receiver=true），因此 ROOT 账号邮箱必有值；
     --   这与"货主端手动创建的子账号邮箱非必填"不矛盾（D1 已定，见 §2.1）
  3. 创建 ROOT 角色（若不存在）:
...
> **★ 变更（2026-09-12，D4 已定）**：新增「报价查询」接口行。**全系统只保留一个统一报价接口** `POST /api/freight-rate/query`，各端以请求体 `caller` 区分（`operator` / `shipper` / `public`）；原 `GET /api/v1/quote/query?caller=...` 与旧货主端 `POST /api/shipper/inquiry/price` **均作废**。
...
- **报价查询接口超时/不可用**（3s）：**★ 变更（2026-09-12）** 查价页提示"系统繁忙，请稍后重试"，**不降级返回缓存价**（价格时效性要求高，返回过期价会造成客诉）
...
| **客户等级变更** | **customer_level 更新** | **★ 变更（2026-09-12）**：**直接影响查价结果**——三张优惠表按「客户等级」匹配的规则可能命中/失效，报价随之变化 | **查价时实时取当前等级**（不缓存等级；`customer` 状态接口每次查价同步调用） |
...
| # | 变更点 | 旧口径 | 新口径 |
|---|--------|--------|--------|
| 1 | 泡比优惠分期 | ~~二期~~ | **一期启用**（阈值阶梯 + 固定减免，多条命中取阈值最大档，与运费优惠叠加） |
| 2 | 优惠对象类型 | ~~一期仅「客户等级」，「客户特批」二期~~ | **一期同时开放「客户特批」+「客户等级」**；**特批 > 等级，高覆盖低不叠加** |
| 3 | 飞书审批编号 | ~~二期开放、一期恒为空~~ | **一期必填（客户特批时）**，**人工录入，不对接飞书审批流** |
| 4 | 结果行字段 | `base_price`/`surcharge_detail`/`final_price` 三字段 | 新增 `service_combination_code` / `delivery_regions` / `freight_discount_detail` / `density_discount_detail` / `surcharge_discount_detail` / `sailing_info`(JSON) |
| 5 | `final_price` 公式 | 公布价 + 附加费 − 优惠 | ~~公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠~~ → **★ 2026-09-12 符号翻转后：`(公布价 + 运费优惠值 + 泡比优惠值) + (附加费合计 + 附加费优惠值合计)`** |
| 6 | 引用表 | 无泡比优惠、无船期 | 新增 `density_discount`、`sailing_schedule` |
| 7 | 邮箱必填（D1） | 待确认 | **已定**：货主端手动创建子账号邮箱非必填；客商中心客户联系人邮箱必填 |
| 8 | 页面名 | 收货地址管理 | **地址管理**（对齐原型） |
| **9** | **优惠值正负号口径（★ 2026-09-12）** | ~~固定减免「正数=减价、负数=加价」，计算 `原金额 − 优惠值`；附加费优惠上限 `优惠值 ≤ 应收单价`~~ | **固定减免「负数=减钱（正向优惠）、正数=加价（反向优惠）」，计算 `原金额 + 优惠值`**；显示：负 `-¥X` / 正 `+¥X` / 0「全免」；附加费优惠上限改为 **`\|优惠值\| ≤ 应收单价`**。**折扣率不变**（乘数，`金额 × 率`） |
...
|---|---------|----------------------|--------------|------|
| ~~E9~~ | ~~超重超长是否在查价阶段计算~~ | — | — | ✅ **已定（2026-09-12）**：**按单箱录入**（询价结果行含长/宽/高）时可算并作**预估展示**（`surcharge_amount` 计入预估、体积重 = 长×宽×高÷计泡系数）；**按总体重量与体积录入**时不计算、不展示。预估**不落库为应付依据**，正式计费以**拣货实测**为准 |
| D1 | 联系人邮箱必填 | 客商中心：客户联系人.email 必填 | 货主端子账号.email 非必填（原型必填项仅 账号/姓/名/性别/手机号） | **✅ 已定（2026-09-12）**：ROOT 账号邮箱来自客户联系人（`is_account_receiver=true`，必填）；**手动创建的子账号邮箱非必填**。二者不矛盾，非缺陷 |
| D2 | ROOT冻结 | 客商中心 R25"关联账户同步冻结" | 货主端"ROOT不可冻结" | **已解决**：ROOT 不可被企业管理员**单独**冻结（前端隐藏按钮），但随客户冻结联动是系统级操作，不受此限制 |
| D3 | 角色表归属 | 客商中心不管理角色 | 货主端角色表引用系统设置模块 | **已对齐**：角色统一由系统设置模块管理，货主端只做引用和分配 |
| ~~D4~~ | ~~报价查询接口路径~~ | — | — | ✅ **已定（2026-09-12，选「甲」）**：统一为 `POST /api/freight-rate/query`，`caller=shipper` 置于请求体；旧两路径作废 |
| **D5** | **优惠后金额的展示顺序** | 超级运价 §7.4：`最终报价 = 折扣后运费 + 优惠后附加费`（先算折扣后运费，再加附加费净额） | 原型费用明细弹窗：~~`公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠`~~ | **★ 变更（2026-09-12，符号翻转）**：货主端统一按 **`(公布价 + 运费优惠值 + 泡比优惠值) + (附加费合计 + 附加费优惠值合计)`** 表述，与超级运价 §7.4 **完全等价**（`折扣后运费` = `公布价 + 运费优惠值 + 泡比优惠值`），**"两种写法"不复存在**。⚠️ 由「待确认」降级为**表述一致性提示**：原型 HTML 仍未按新符号口径改造（原型不在本次文档改动范围） |
| **D6** | **附加入库/账单的叠加口径** | 超级运价 §7.4 ~~`折扣后运费 = 公布价 − Σ运费固定减免 − Σ泡比固定减免`~~ → **★ 2026-09-12：`折扣后运费 = 公布价 + Σ运费优惠值 + Σ泡比优惠值`**（固定减免已是带符号值，不再用减号） | 原型 ~~`finalUnit = afterFreight + scTotal − ddVal`（先减运费优惠得 `afterFreight`，加附加费净额，最后减泡比优惠）~~ | **★ 变更（2026-09-12，符号翻转）**：正确口径为 `finalUnit = (公布价 + 运费优惠值 + 泡比优惠值) + (scTotal + 附加费优惠值合计)`——先算完**运费侧**，再单独加**附加费侧**。原型现写法把泡比优惠放在最后扣减、**跨段抵扣附加费**，与「泡比优惠只作用运费侧、不得抵扣附加费」硬约束冲突，**已判为口径错误（不只是展示顺序问题）**，归入"原型已修复（2026-09-12）"，不在本次文档改动范围 |
| **D7** | **原型缺页** | — | 菜单已挂、**页面文件不存在**：`我的订单.html` / `草稿箱.html` / `异常订单.html` / `发票管理.html` | **如实标注为「菜单已挂、页面待建」**，不计入已完成功能。**⚠️ 待确认**：是否本期建设 |
