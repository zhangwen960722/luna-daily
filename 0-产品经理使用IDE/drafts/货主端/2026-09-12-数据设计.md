...
> 账单管理 / 资金账户 **归「财务」模块**（实体见 `drafts/财务/2026-09-18-数据设计.md`）、后台任务 **归「后台任务」模块**（见 `drafts/后台任务/`）——均按「**引用其他模块页面**」处理，**货主端不定义实体**，上表按引用登记即可。
...
| D7 | 原型缺页 | — | **页面文件不存在、菜单已摘除**：`我的订单.html` / `草稿箱.html` / `异常订单.html` / `发票管理.html` | **如实标注为「页面待建、菜单已摘除」**，不计入已完成功能；本期不建设，**后续做到该块时自然补充并挂回菜单** |
