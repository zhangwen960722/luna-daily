...
| `freight_discount_detail` | 运费优惠 | JSON | No | -- | 命中规则名 + 减免额；**客户特批 > 客户等级，高覆盖低不叠加** |
| `density_discount_detail` | 泡比优惠 | JSON | No | -- | 命中规则名 + 减免额；泡比≥阈值，**多条命中取阈值最大档**；**一期正常展示，仅 KG 计价行** |
| `surcharge_detail` | 附加费 | JSON | Yes | -- | 逐条快照附加费名称+原始单价+优惠后单价 |
| `surcharge_discount_detail` | 附加费优惠 | JSON | No | -- | 命中规则名 + 减免额；与运费优惠独立计算 |
| `final_price` | 最终报价 | Decimal(18,2) | Yes | -- | 公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠 |
...
- `price_inquiry_result.freight_discount_detail` / `density_discount_detail` / `surcharge_discount_detail`：**★ 变更（2026-09-12）** 三张优惠表各自的命中规则名+减免额，用 JSON 快照，便于追溯"当时按哪条规则算的"
...
> **★ 变更（2026-09-12）** 三层询价数据安全（超级运价 PRD R27）：同一报价 API `GET /api/v1/quote/query`，`caller=shipper` 时**只返回公布价+优惠，不返回成本底价/利润/盈亏标识**；未登录 + `caller=shipper` → 401。
...
| 优惠方式 | 一期仅 20 固定减免 | 一期仅 20 固定减免 | 折扣率/固定减免 |
...
# 4. 最终报价（超级运价 PRD §7.4）
最终报价 = 公布价 − 运费优惠 + Σ(附加费原价 − 附加费优惠) − 泡比优惠
约束：任一费用项最终金额不得为负（优惠上限截断 R31）
```

> **⚠️ 未决项（见 §8 不一致处理标注）**：
> - **E9 — 超重超长是否在查价阶段参与计算**：超级运价定稿口径为「超重超长只在**拣货**触发」（依据拣货实测尺寸/重量；RDD §1656 明确「运价查询阶段不计算」），但货主端原型 `查价订舱.html` **当前仍在查价阶段依据客户自填尺寸/货物属性计算并展示**「超重件加收 200/件」「超重超长 200/箱」。**两种口径（A 查价阶段彻底不算 / B 保留为预估但不落库、不进账单）待产品确认，本文档不预设结论。**
> - **附加费优惠与泡比优惠的叠加顺序**：超级运价 PRD §7.4 的公式为 `最终报价 = 折扣后运费 + 优惠后附加费`，而原型费用明细弹窗展示为 `公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠`。两者计算结果一致（减法可交换），但**展示顺序不一致**，已在 §8 列出待确认。
...
| **DiscountMode** | 10 | RATE | 折扣率（二期开放） | 三张优惠表 |
| **DiscountMode** | 20 | FIXED | 固定减免（一期） | 三张优惠表 |
...
> **★ 变更（2026-09-12）**：新增 `DiscountTargetType` / `DiscountMode` / `ValidityType` / `FeeDirection` 四组枚举。要点：① `DiscountTargetType` **一期同时开放 10 与 20**（原「一期仅 20 客户等级」作废）；② `DiscountMode` 一期仅 20；③ `ValidityType` 一期 10/20；④ `FeeDirection` 固定 10。
...
| **报价查询** | **POST** | **/api/freight-rate/query** 或 **GET /api/v1/quote/query?caller=shipper** | **超级运价** | **货主端** | — | **3s** |

> **★ 变更（2026-09-12）**：新增「报价查询」接口行。超级运价 PRD 同时出现两个路径（§5.12 关联接口 `POST /api/freight-rate/query`、§R27 `GET /api/v1/quote/query`），**两者口径不一致，待确认以哪个为准**（已列入 §8）。
...
| 5 | `final_price` 公式 | 公布价 + 附加费 − 优惠 | **公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠** |
...
|---|---------|----------------------|--------------|------|
| **E9** | **超重超长是否在查价阶段计算** | 超级运价已定：**超重超长只在「拣货」触发**（需拣货实测尺寸/重量）；RDD 明确「**运价查询阶段不计算**」 | `查价订舱.html` **当前按客户自填尺寸/货物属性在查价阶段即计算并展示**「超重件加收 200/件（按件）」「超重超长 200/箱（按箱）」；2026-09-12 已在报价结果区上方加提示条「超重超长费用将在拣货实测后计算……实际费用以实测结果为准」 | **⚠️ 待产品确认，本文档不预设结论。** 候选：**(A)** 查价阶段彻底不算超重超长；**(B)** 保留为"预估展示"但**不落库、不进账单**。确认后需同步修改本文件 §2.4 `surcharge_detail` 口径 |
...
| **D4** | **报价查询接口路径** | 超级运价 PRD §5.12 写 `POST /api/freight-rate/query`；§R27 写 `GET /api/v1/quote/query?caller=shipper` | 货主端原 PRD 写 `POST /api/shipper/inquiry/price` | **⚠️ 待确认**：三个路径并存，需产品/研发确认货主端对接的唯一接口。本文档不自行选定 |
| **D5** | **优惠后金额的展示顺序** | 超级运价 §7.4：`最终报价 = 折扣后运费 + 优惠后附加费`（先算折扣后运费，再加附加费净额） | 原型费用明细弹窗：`公布价 − 运费优惠 + 附加费(优惠后) − 泡比优惠` | **数值等价、展示顺序不同**（减法可交换）。**⚠️ 待确认**：以哪个公式作为"研发唯一口径"表述 |
| **D6** | **附加入库/账单的叠加口径** | 超级运价 §7.4 `折扣后运费 = 公布价 − Σ运费固定减免 − Σ泡比固定减免` | 原型 `finalUnit = afterFreight + scTotal − ddVal`（先减运费优惠得 `afterFreight`，加附加费净额，最后减泡比优惠） | 同 D5，数值等价。**⚠️ 待确认**：泡比优惠在"是否可抵扣附加费"上两种写法都成立（因为都是纯减法），但若后续引入"附加费不可被泡比优惠抵扣"的业务约束则二者不等价 |
