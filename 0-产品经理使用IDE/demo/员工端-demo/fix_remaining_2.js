...
['超级运价_工作台.html', '运价维护_主列表.html', '超级运价_运价表维护.html'].forEach(f => {
    const p = path.join(dir, f);
    if (!fs.existsSync(p)) return;
    
    let content = fs.readFileSync(p, 'utf8');
    
    // 如果包含 top-nav，我们执行同样的替换操作
    if (content.includes('class="top-nav"')) {
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        ...
        console.log('Processed structure for:', f);
    } else {
        console.log('No top-nav found or already processed for:', f);
    }
});
