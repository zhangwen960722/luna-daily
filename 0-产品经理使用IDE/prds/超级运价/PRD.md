...
> **文档类型**：PRD | **版本**：V2.2 | **状态**：草稿 | **日期**：2026-06-06
> **作者**：AI PM + 飞点业务团队
> **评审人**：产品/研发/测试/业务代表
...
| V2.2 | 2026-06-06 | 全局实体抽象 + 利润策略重构 + 周批次快照模型 + 成本底价计算对齐 | AI PM |

### 0.2 关联链接

- RDD：`drafts/2026-05-25-超级运价需求分析/RDD.md`（v4.0）
...
| 利润策略 | 独立全局实体，百分比。渠道/组合引用策略ID。周批次快照，运价编辑时初始填充 | P0 | 全局配置页（成本管理内） | 独立表 profit_strategy |
...
**本周运价 Tab**（两级折叠视图）：
按 `(服务组合, 仓库组/邮编前缀, 交货地)` 分组为主行，展开显示各重量段子行。成本底价引用自成本管理（只读），初始公布价 = 成本底价 × 默认利润率填充所有重量段（初始值相同），运价管理员逐行手工覆盖公布价。利润策略为全局独立实体，渠道/组合引用。
...
利润策略为独立表 `profit_strategy`，不作为成本项的内联字段。成本和利润分离——成本管理只管成本，利润策略独立配置。

| 字段名 | 中文名 | 类型 | 必填 | 校验规则 |
|--------|--------|------|------|---------|
| name | 策略名称 | 字符串(30) | ✅ | 如"全段默认""分段默认" |
| profit_rate | 默认利润率 | 小数(5,1) | ✅ | 如 15 = 15% |
| applicable_scope | 适用范围 | 小整数 | ✅ | 10:全段服务 20:分段服务 30:通用 |
| is_enabled | 是否启用 | 布尔 | ✅ | |

**数据流**：全局策略 → 渠道创建时自动匹配（根据 channel_type） → 组合从渠道继承策略ID → 周批次快照利润率 → 运价编辑初始填充（所有重量段初始值相同，运价管理员逐行覆盖）。

> **设计决策**：利润策略从成本项剥离。1) 成本项是公共底座不应带偏向性配置；2) 利润差异来自重量段×仓点，不是成本项；3) 策略只给起步值，最终公布价永远是人工决定。

---

### 5.9 运价维护（工作台运价 Tab）

**页面信息**：运价更新工作台内嵌，本周运价 Tab。按 (服务组合, 仓库组/邮编前缀, 交货地) 分组两级折叠视图。展开显示各重量段子行：成本底价（参照），初始公布价（= 成本底价 × 利润率），公布价（可编辑），利润差 + 盈亏色标。

**新建运价行**：选择组合 + 维度 → 自动为所有重量段创建行，初始公布价 = 成本底价 × 默认利润率。

**编辑弹窗**：上半部展示成本底价参照（头程合计 + 派送MAX，只读）+ 默认利润率输入（调后即时重算全部段初始价）。下半部逐重量段表格：成本底价 | 初始公布价 | 公布价输入 | 利润差 | 偏离标记。

**新建周批次**：从全局表快照折算基准 + 重量段 + 利润率；可选从上周复制船期 + 运价行。发布后锁定不可编辑。
...
头程共享 每公斤:
  Σ( 非派送段启用项的 单价 × (if USD: 批次汇率 else 1) ) ÷ 批次快照.head_kg_standard

头程共享 每立方米:
  Σ( 非派送段启用项的 单价 × (if USD: 批次汇率 else 1) ) ÷ 批次快照.head_cbm_standard

派送段普通项 每公斤（提拆派费等，不区分仓点）:
  Σ( 派送段非仓点启用项的 单价 × (if USD: 批次汇率 else 1) ) ÷ 批次快照.head_kg_standard
