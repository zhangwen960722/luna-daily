...
**导出动作本身的长流程（异步任务）**：导出量大时不要在页面直出 —— 走「提交任务 → **后台任务**页看进度 → 下载结果文件」（先例：账单导出，见财务产品文档 R2.12 / 后台任务产品文档 §导出任务）：① 页面只负责提交 + 给一句"到哪看"的 Toast，**不阻塞、不跳转**；② 结果文件在后台任务**一个下载入口**，不在业务列表里堆 N 个下载按钮。
