...
> 本模板由 `requirement-clarifier` Skill 的 §Output 引用。生成 RDD 时按此结构逐章填充。

> ## ⚠️ 生成边界（铁律 · 见 AGENTS.md §文档职责边界）
>
> 本卡片只写「**要做什么 / 为什么 / 验收**」。以下内容**不写正文、只引用**：
> - **字段结构 / 类型 / 约束** → 数据设计 §Schema（RDD 字段表只列字段名与业务含义，不逐字段重复定义类型/约束）
> - **业务规则 / 校验逻辑 / 计算公式** → PRD §6 / §7（RDD 只写「见 PRD Rxx」；业务规则**不编号**，确需编号带 `BPR-` / `RR-` 前缀）
> - **状态机 / 枚举字典** → PRD §2 / §3
> - **功能清单 / 页面清单** → PRD §4（RDD 不重复列）
