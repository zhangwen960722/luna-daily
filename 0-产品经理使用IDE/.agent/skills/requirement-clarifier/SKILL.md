...
请按 [RDD 模板](0-产品经理使用IDE\templates\rdd-template.md) 输出需求定义卡片，完整章节如下：
...
### 2. 用户故事 (User Story)
- **基础实体**：先定义核心实体（字段表 / 关联图 / 校验规则），再展开 Epic
- **Epic 结构**：每个 Epic = As a / I want / So that + 核心模型（ASCII 图）+ 核心场景列表
- **关键**：不要直接跳到功能列表，先建立用户对实体关系的认知
...
> **输出原则**：先搭结构再填内容。基础实体是骨架，Epic 是血肉，功能清单是索引。避免"一上来就写功能列表"——没有实体关系认知的功能列表 = 无源之水。
